function SBJ12a_corr_ANOVA_ts_cluster_plotOnly(SBJ,pipeline_id,stat_id,clust_id,an_id,roi_id,atlas_id,plt_id,fig_vis,save_fig,plot_out)
...
clust_colors = [0 0 1; 0 1 1; 1 0 1; 1 0 0];  %garish blue, cyan, magenta, red
clust_names = {'Early','Reponse','Post-Resp','Late'};
...
%                     'Color',[0.4 0.4 0.4],'LineStyle','--','LineWidth',2);
    ...
    if strcmp(clust_vars.clust_method,'kmeans')
        ...
        for clust_ix = 1:n_clust
            ...
            if cond_ix<=numel(grp_lab)
                plot(win_center,centroids{cond_ix}(clust_ix,:),...
                    'Color',clust_colors(time_bin,:),'LineStyle','-','LineWidth',2);
            else
                plot(1:numel(stat.time),centroids{cond_ix}(clust_ix,:),...
                    'Color',clust_colors(time_bin,:),'LineStyle','-','LineWidth',2);
            end
        end
        legend(clust_names{clust_bin{cond_ix}+1});
