...
!!! need to get the power, not just filter!!!
% Cut into Trials
...
for an_ix = 1:numel(cond_lab)
    ...
    roi_hfa{an_ix} = ft_timelockanalysis(cfgavg,roi_trl);
    % Grab n_trials for design matrix
    n_trials(an_ix) = size(roi_hfa{an_ix}.trial,1);
end
...
%% Save Results
if save_data
    data_out_filename = strcat(SBJ_dir,'04_proc/',SBJ,'_HFA_stats_',conditions,'_ROI_',event_type,'.mat');
    fprintf('Saving %s\n',data_out_filename);
