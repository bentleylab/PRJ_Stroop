function GetBaselineBlockEncodingElecs(SBJ, proc_id)
    ...
    %% Trial labels
    trials = fn_create_PERM_design(trial_info);
    ...
    hfa = extractHFA(data,0,0,1000,trial_info); % extract 70-150 Hz HFA and z-score to -0.5:-0.2s using boostrapping
    ...
    % Remove errors
    hfa.ztrial([trials.error] == 1,:,:) = [];
    trials([trials.error] == 1,:) = [];
    ...
    conditions = {'BlockType','pTrialType'};
            ...
            case 'BlockType'
                ...
                for chani = 1:length(w2{condi}.label)
                    ...
                    for jj = 1:islands.NumObjects % loop through non-zero traces
                        ...
                        if all(w2{condi}.time(islands.PixelIdxList{jj}) > 0) && numel(islands.PixelIdxList{jj}) > ceil(sum(w2{condi}.time > 0) * min_pc)
    ...
    save_filename = fullfile(save_dir, [SBJ '_smANOVA_ROI_pCNI_PC_HGh_R5t1_zbt_WL05_WS002.mat']);

    save(save_filename,'w2','hfa','trials','-v7.3');
