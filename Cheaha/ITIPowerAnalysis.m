function ITIPowerAnalysis(SBJ, proc_id, an_id,cond)
    ...
    tfr = zbaseline_tfr(tfr,tfr.time, -1.5, 1.5, 1000);
    ...
    times2save = -1.5:.025:0; % in seconds
    times2plot = -1.5:.01:0; % in seconds
    ...
    if strcmpi(an.evnt_lab,'S')
        ...
        save_dir = strcat(root_dir, 'PRJ_Stroop/results/Power/ITIStim/');
        ...
        power_save_filename = fullfile(save_dir, [SBJ '.mat']);
         ...
         % Save the results for each subject
        save_dir = strcat(root_dir, 'PRJ_Stroop/results/Power/ITIResp/');
