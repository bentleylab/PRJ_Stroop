function B00_RT_SBJ_hist(SBJ,save_fig,fig_vis,fig_type)
...
figure('Name',fig_name,'units','normalized','outerposition',[0 0 1 1],'Visible',fig_vis);hold on;
...
trial_legend = {};
for lab_ix = 1:length(trial_lab)
    trial_legend{lab_ix} = [trial_lab{lab_ix} '-' num2str(length(trial_RTs{lab_ix}))];
end
legend(trial_legend);
...
for b_ix = 1:length(block_lab)
    ...
    for lab_ix = 1:length(trial_lab)
        block_RTs{b_ix,lab_ix} = RTs(fn_condition_index([trial_lab{lab_ix} '_' block_lab{b_ix}],...
...
for b_ix = 1:length(block_lab)
    ...
    for lab_ix = 1:length(trial_lab)
        line([mean([block_RTs{b_ix,lab_ix}]) mean([block_RTs{b_ix,lab_ix}])], ylim,...
            'Color', [trial_colors{lab_ix}], 'LineWidth', line_w);
        trial_legend{lab_ix} = [trial_lab{lab_ix} '-' num2str(length(block_RTs{b_ix,lab_ix}))];
    end
    legend(trial_legend);
