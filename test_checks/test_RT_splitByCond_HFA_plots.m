...
%% Split trials by RT
split_cut = zeros([numel(cond_lab) RT_split+1]);
split_idx = cell(size(cond_lab));
split_cnt = zeros([numel(cond_lab) RT_split+1]);
split_avg = zeros([numel(cond_lab) RT_split]);
for cond_ix = 1:numel(cond_lab)
    split_cut(cond_ix,:) = quantile(rt_cond{cond_ix},0:1/RT_split:1);
    split_cut(cond_ix,end) = split_cut(cond_ix,end)+1;
    [split_cnt(cond_ix,:), split_idx{cond_ix}] = histc(rt_cond{cond_ix},split_cut(cond_ix,:));
...
for split_ix = 1:RT_split
    ...
    plot_info.ax     = gca;
    plot_info.title  = ['spilt ix = ' num2str(split_ix)];%[event_lab{set_ix} '-Locked'];
    plot_info.legend = 1;
