function fn_view_recon_group(SBJs, pipeline_id, plot_type, reg_type, show_labels, hemi)%, view_angle)
...
%   FORCED: view_space [str] - exclusively 'mni' (obviously...)
...
%% Load and combine elec structs
SBJ_colors = distinguishable_colors(numel(SBJs));
all_colors = [];
elec = cell([numel(SBJs) 1]);
any_seeg = 0;
for sbj_ix = 1:numel(SBJs)
    SBJ_vars_cmd = ['run ' root_dir 'PRJ_Stroop/scripts/SBJ_vars/' SBJs{sbj_ix} '_vars.m'];
    eval(SBJ_vars_cmd);
    
    tmp = load([SBJ_vars.dirs.recon,SBJs{sbj_ix},'_elec_',pipeline_id,'_mni',reg_suffix,'.mat']);
    elec{sbj_ix} = tmp.elec; clear tmp SBJ_vars_cmd SBJ_vars;
    for e_ix = 1:numel(elec{sbj_ix}.label)
        elec{sbj_ix}.label{e_ix} = [SBJs{sbj_ix} '_' elec{sbj_ix}.label{e_ix}];
    end
    if any(strcmp(elec{sbj_ix}.chantype,'seeg'))
        any_seeg = 1;
    end
    all_colors = vertcat(all_colors,repmat(SBJ_colors(sbj_ix,:),[numel(elec{sbj_ix}.label) 1]));
end

elec = ft_appendsens([],elec{:});
elec.color = all_colors;    % ft_appendsens delets the .color field otherwise...
...
if strcmp(plot_type,'3d')
    ...
    if show_labels
        ft_plot_sens(elec, 'elecshape', 'point', 'marker', 'o', 'facecolor', elec.color, 'elecsize', 15, 'label', 'label');
    else
        ft_plot_sens(elec, 'elecshape', 'point', 'marker', '.', 'facecolor', elec.color, 'elecsize', 15);
    end
