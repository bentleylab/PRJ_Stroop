function fn_plot_ts_but(plot_info, trials, event, cond, cond_ix)
% Butterfly plot of single condition with mean overlayed on given plot handle
% INPUTS:
...
%       .but_width [int] - line width of the individual trials
%       .but_color [cell str array] - strings of colors for individual trials
%       .but_alpha [int] - transparency of the individual trials
%       .mean_width [int] - line width of the mean trace
%   trials [matrix] - trial by time matrix to be plotted; rows = time series already trimmed to plot limits
...
%   cond_ix [int] - index of condition in cond struct
% TO BE ADDED:
...
% Plot Trials
hold on;
t = linspace(1,size(trials,2),size(trials,2));
for t_ix = 1:size(trials,1)
    patchline(t,trials(t_ix,:),'EdgeColor',plot_info.but_color,'LineWidth',plot_info.but_width,...
                                'EdgeAlpha',plot_info.but_alpha);
end

% Plot Event Locked Average
mean_line = plot(mean(trials,1),'Color',[cond.color{cond_ix}],'LineStyle',cond.style{cond_ix},...
    'LineWidth',plot_info.mean_width);
main_lines = [mean_line];
% ylim(ylims);
...
% Axes and Labels
plot_info.ax.XLim = [0,size(trials,2)];
plot_info.ax.XTick = 0:plot_info.x_step:size(trials,2);
plot_info.ax.XTickLabel = plot_info.x_lab;
title(plot_info.title);
if plot_info.legend==1
    legend(main_lines,cond.name{cond_ix},event.name{:},'Location',plot_info.legend_loc);
end
