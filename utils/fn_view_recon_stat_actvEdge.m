function fn_view_recon_stat_actvEdge(SBJ, pipeline_id, stat_id, an_id, view_space, reg_type, show_labels, hemi)
...
%% Load Active HFA Stats
load([SBJ_vars.dirs.proc SBJ '_actv_ROI_' an_id '_mn100.mat']);
% Re-order elec to match the data loaded
elec = fn_reorder_elec(elec,hfa.label);

edge_colors = cell([numel(hfa.label) 1]);
for ch_ix = 1:numel(hfa.label)
    if any(strcmp(actv_ch,hfa.label{ch_ix}))
        ...
        if any(epoch_signs==1) && any(epoch_signs==-1)
            edge_colors{ch_ix,1} = [0.75 0 0.75];           % Purple for both +/-
        elseif any(epoch_signs==1)
            edge_colors{ch_ix,1} = [1 0 0];                 % Red for +
        elseif any(epoch_signs==-1)
            edge_colors{ch_ix,1} = [0 0 1];                 % Blue for -
        end
    else
        edge_colors{ch_ix,1} = ns_color;
    end
end
...
load(f_name,'stat','w2');
% Check matching order of actv(/elec) and ANOVA labels
[~,order_idx] = ismember(stat.label,hfa.label);
if sum(order_idx==[1:numel(stat.label)]')<numel(stat.label)
    error('order of actv HFA and stat ANOVA labels does not match!');
end
...
for grp_ix = 1:numel(grp_lab)+1
    if grp_ix<=numel(grp_lab)
        plot_name = [SBJ '_ANOVA.actv_' grp_lab{grp_ix} '_' stat_id '_' an_id];
    else
        plot_name = [SBJ '_ANOVA.actv_' rt_lab{1} '_' stat_id '_' an_id];
    end
    ...
    for e = 1:numel(elec.label)
        ...
        eplot = ft_plot_sens(elec_tmp, 'style', 'ko');
        set(eplot, 'MarkerFaceColor', elec_colors{e,grp_ix}, 'MarkerEdgeColor', edge_colors{e,1}, 'MarkerSize', 10);
        %         ft_plot_sens(elec_tmp, 'elecshape', 'point', 'label', lab_arg, 'elecshape', 'circle',...
        %             'facecolor', elec_colors{e,grp_ix}, 'edgecolor', edge_colors{e,1}, 'edgealpha', 0.3);
    end
