function test_grp_ROI(SBJs, proc_id, reg_type, show_labels,...
                                 hemi, atlas_id, roi_id, plot_roi, mirror, custom_roi, varargin)
...
for sbj_ix = 1:numel(SBJs)
    ...
    correctROI_elecfname = [SBJ_vars.dirs.recon,SBJ,'_elec_',proc_id,'_pat','_',atlas_id,'_final_paper.mat'];
    
    correctelec = load(correctROI_elecfname);
    tmp = load(elec_fname); 

    tmp.elec.ROI = correctelec.elec.ROI; tmp.elec.gROI = correctelec.elec.gROI;
    ...
    for roi_ix = 1:numel(plot_roi_list)
        if strcmpi(custom_roi,'dmPFC')
            plot_elecs(:,roi_ix) = strcmp(elec{sbj_ix}.(roi_field),'aMCC') | strcmp(elec{sbj_ix}.(roi_field),'SMC');
        elseif strcmpi(custom_roi,'dlPFC')
            plot_elecs(:,roi_ix) = strcmp(elec{sbj_ix}.(roi_field),'dlPFC');
        end
    end
