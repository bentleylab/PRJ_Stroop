...
% Prep report
out_dir = [root_dir 'PRJ_Stroop/results/HFA/'];
...
%         end
    ...
    elseif strcmp(stat_id,'CSE')
        ...
        for ch_ix = 1:numel(stat.label)
            ...
            for cond_ix = 1:numel(cond_lab)-1
                if any(qvals(cond_ix,:)<0.05,2)
                    sig_ch{cond_ix} = [sig_ch{cond_ix} {[SBJs{sbj_ix} '_' w2.label{ch_ix}]}];
                end
            end
            if any(stat.mask(ch_ix,1,:))
                sig_ch{cond_ix+1} = [sig_ch{cond_ix+1} {[SBJs{sbj_ix} '_' stat.label{ch_ix}]}];
            end
        end
        clear stat w2
    end
    ...
    for cond_ix = 1:numel(cond_lab)
        ...
        if numel(intersect(sig_ch{cond_ix},[atlas_out_elecs; hemi_out_elecs]))==numel(sig_ch{cond_ix})
            ...
            cfgs = [];
            cfgs.channel = [sig_ch{cond_ix} fn_ch_lab_negate(atlas_out_elecs) fn_ch_lab_negate(hemi_out_elecs)];
