...
switch factor_name
    ...
    case 'pCNI'
        ...
        line_styles = {'-', '-', '-'};    % colors for cond_lab plotting
    case 'SDR'
        labels = {'S', 'D', 'R'};   % light blue, gold, pink/red (vivid)
        colors = {[3 209 255]./256, [255 205 3]./256, [255 3 108]./256};
        line_styles = {'-', '-', '-'};
    case 'CNI_p_PC'
        labels = {'CNI', 'pCNI', 'PC'};   % gold, teal, pink (light but sharp)
    ...
    case 'CI'
        ...
        line_styles = {'-', '-'};    % colors for cond_lab plotting
    case 'PC'
        labels = {'MC', 'EQ', 'MI'};    % green, orange, purple (darkish)
