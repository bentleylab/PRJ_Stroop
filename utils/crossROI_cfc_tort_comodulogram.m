function [comdlgrm, comdlgrm_z, phase2power] = crossROI_cfc_tort_comodulogram(dataMatphase,dataMatamplitude,srate,n_surrogates,n_bins,LF_steps,LF_bw,HF_steps,tcutEdge)

%% [comdlgrm, comdlgrm_z] = cfc_tort_comodulogram(LFPphase,LFPamplitude,fs,n_surrogates,n_bins,LF_steps,LF_bw,HF_steps,tcutEdge)
%
% Compute PAC comodulogram with Tort's method (Tort et al, 2009, 2010) in a single channel and condition
%
% Input
% dataMatphase: 2d matrix containing time samples of interest per trial (samples x trials) for the phase modulating region
% dataMatamplitude: 2d matrix containing time samples of interest per trial (samples x trials) for the amp modulated region
% srate: sampling rate
...
%% defaults
if nargin < 4 || isempty(n_surrogates)
...
n_trials       = size(dataMatphase,2);
n_samples_long = size(dataMatphase,1);
