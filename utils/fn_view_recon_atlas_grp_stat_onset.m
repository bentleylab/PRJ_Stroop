...
if n_tbins==4 && ~strcmp(tbin_id,'cnts')
...
for sbj_ix = 1:numel(SBJs)
    ...
    elec_sbj{sbj_ix,cond_ix}.onset = NaN(size(elec_sbj{sbj_ix,cond_ix}.label));
    ...
    for elec_ix = 1:numel(elec_sbj{sbj_ix,1}.label)
        stat_ix = strcmp(stat.label,elec_sbj{sbj_ix,1}.label);
...
%                 sig_ch{sbj_ix,grp_ix} = [sig_ch{sbj_ix,grp_ix} stat.label{ch_ix}];
                sig_onsets = stat.time(win_lim(squeeze(qvals(grp_ix,stat_ix,:))<0.05,1));
