...
function [freq, causality, COH, iCOH, power, S] = compute_nonparGGC_wavelet_Morlet(X, fs, freq, pad0, flgCond, omega0)
%==========================================================================
...
% - omega0:     central frequency parameter in Morlet wavelet (a choice of omega0=(8*pi^2-1)/(4*pi) sets fourier-factor=1)
%--------------------------------------------------------------------------
% OUTPUT
% - freq:       frequencies at which causality, coherence and power are computed (vector)
...
% [4] Morlet et al., Geophysics, 1982.
% [5] Torrence & Compo, Bull. Am. Meteorol. Soc., 1998.
% 
...
if nargin < 6,  omega0 = 6;     end;                                        % central frequency parameter in Morlet wavelet



%--- Spectral quantities ----------
if nargout == 2, 
    S = xwt_cmorl_nv(X,fs,freq,pad0,omega0);   % S: 4-D spectral matrix
end

if nargout == 3,
    [S, COH] = xwt_cmorl_nv(X,fs,freq,pad0,omega0);
elseif nargout > 3,
    [S, COH, iCOH] = xwt_cmorl_nv(X,fs,freq,pad0,omega0);
    if nargout > 4
        for ichan = 1:Nc,
...
%--- Anas's addition ---------
% Try smoothing spectral matrix before factorization to prevent failure of
% cholesky factorization due to non positive definite matrix

gausskernelT = exp(-(-4*25:4*25).^2 / (2 * 25^2)); % Create Gaussian kernel
gausskernelT = gausskernelT / sum(gausskernelT); % Normalize the Gaussian window

for chani = 1:Nc
    for chanj = 1:Nc
        slicedS = squeeze(S(chani,chanj,:,:));
        % time smooth
        slicedS_time = conv2(slicedS,gausskernelT,'same');
        % frequency smooth
        slicedS_time_freq = smoothdata(slicedS_time,1,"movmean",5);

        % figure(1)
        % contourf(1:Nt,freq1,abs(slicedS),50,'EdgeColor','none')
        % figure(2)
        % contourf(1:Nt,freq1,abs(slicedS_time),50,'EdgeColor','none')
        % figure(3)
        % contourf(1:Nt,freq1,abs(slicedS_time_freq),50,'EdgeColor','none')
        S(chani,chanj,:,:) = slicedS_time_freq;
    end
end
