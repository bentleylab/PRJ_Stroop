function SBJ10c_HFA_GRP_plot_SR_ROIavg_errbr_RTcorr_ANOVA(SBJs,stat_id,pipeline_id,an_id_s,an_id_r,...
                                                roi_id,plt_id,save_fig,fig_vis,fig_filetype)%,butterfly
!!! looks shitty
...
% Set up onset counts
ts       = {cell([numel(roi_list) numel(grp_lab)+1])...
            cell([numel(roi_list) numel(grp_lab)+1])};
sig_idx  = {cell([numel(roi_list) numel(grp_lab)+1])...
            cell([numel(roi_list) numel(grp_lab)+1])};
elec_roi_cnt  = zeros([2 numel(roi_list)]);
        ...
        for ch_ix = 1:numel(stat{sr_ix}.label)
            ...
            if strmatch(einfo(einfo_ix,einfo_roi_col),roi_list,'exact')
                ...
                if sum(squeeze(stat{sr_ix}.mask(ch_ix,1,:)))>0
                    sig_idx{sr_ix}{roi_ix,numel(grp_lab)+1} = [...
                        sig_idx{sr_ix}{roi_ix,numel(grp_lab)+1}; ...
                        squeeze(stat{sr_ix}.mask(ch_ix,:,:))'];
                    ts{sr_ix}{roi_ix,numel(grp_lab)+1} = [...
                        ts{sr_ix}{roi_ix,numel(grp_lab)+1}; ...
...
for sr_ix = 1:2
    for roi_ix = 1:numel(roi_list)
        for cond_ix = 1:numel(grp_lab)+1
            if cond_ix<=numel(grp_lab)
                % Average ANOVA factors
                ts_avg{sr_ix}{roi_ix,cond_ix} = mean(ts{sr_ix}{roi_ix,cond_ix},1);
            else
                % Average RT pos, neg, pos/neg separately
                ts_avg{sr_ix}{roi_ix,cond_ix} = NaN([3 size(ts{sr_ix}{roi_ix,cond_ix},2)]);
                rt_type_idx = zeros([1 size(ts{sr_ix}{roi_ix,cond_ix},1)]);
                for ch_ix = 1:size(ts{sr_ix}{roi_ix,cond_ix},1)
                    sig_chunks = fn_find_chunks(squeeze(sig_idx{sr_ix}{roi_ix,cond_ix}(ch_ix,:)));
                    sig_chunks(squeeze(sig_idx{sr_ix}{roi_ix,cond_ix}(ch_ix,sig_chunks(:,1)))==0,:) = [];
                    ch_type = [0 0]; %pos, neg
                    for sig_ix = 1:size(sig_chunks,1)
                        sig_sign = mean(ts{sr_ix}{roi_ix,cond_ix}(ch_ix,sig_chunks(sig_ix,1):sig_chunks(sig_ix,2)));
                        if sig_sign>0
                            ch_type(1) = 1; % pos
                        elseif sig_sign<0
                            ch_type(2) = 1; % neg
                        else
                            error('Really? ts_avg is really exactly zero???'); % neither?
                        end
                    end
                    if all(ch_type)             % both
                        rt_type_idx(ch_ix) = 3;
                    elseif ch_type(1)           % pos
                        rt_type_idx(ch_ix) = 1;
                    elseif ch_type(2)           % neg
                        rt_type_idx(ch_ix) = 2;
                    else
                        error('How is this significant?');
                    end
                end
                
                % Compute Average and StErr
                for type_ix = 1:3
                    ts_avg{sr_ix}{roi_ix,cond_ix}(type_ix,:) = ...
                        mean(ts{sr_ix}{roi_ix,cond_ix}(rt_type_idx==type_ix,:),1);
                    ts_var{sr_ix}{roi_ix,cond_ix}(1,:) = ...
                        std(ts{sr_ix}{roi_ix,cond_ix}(rt_type_idx==type_ix,:),[],1)...
                        ./sqrt(sum(rt_type_idx==type_ix));
                end
            end
        end
    end
...
for grp_ix = 1:numel(grp_lab)   %shouldn't need the vertcat here anymore, but easier to keep it
    max_w2 = max([max(max(vertcat(ts_avg{1}{:,grp_ix}))) ...
        max(max(vertcat(ts_avg{2}{:,grp_ix})))]);
    min_w2 = min([min(min(vertcat(ts_avg{1}{:,grp_ix}))) ...
        min(min(vertcat(ts_avg{2}{:,grp_ix})))]);
...
max_rho = max([max(max(vertcat(ts_avg{1}{:,numel(grp_lab)+1}))) ...
    max(max(vertcat(ts_avg{2}{:,numel(grp_lab)+1})))]);
min_rho = min([min(min(vertcat(ts_avg{1}{:,numel(grp_lab)+1}))) ...
    min(min(vertcat(ts_avg{2}{:,numel(grp_lab)+1})))]);
...
for cond_ix = 1:numel(cond_lab)
    ...
    fig_name = ['GRP_ANOVA_' stat_id '_SR_' roi_id 'avg_' cond_lab{cond_ix}];
    ...
    for sr_ix = 1:2
        ...
        % Plot effect size average per ROI
        for roi_ix = numel(roi_list):-1:1 % OFC first to be in background
            if strcmp(cond_lab{cond_ix},'RT') && ~isempty(ts_avg{sr_ix}{roi_ix,cond_ix})
                for rt_type = 1:3
                    tmp_line = shadedErrorBar(1:numel(ts_avg{sr_ix}{roi_ix,cond_ix}(rt_type,:)),...
                        ts_avg{sr_ix}{roi_ix,cond_ix}(rt_type,:),ts_var{sr_ix}{roi_ix,cond_ix}(rt_type,:),...
                        {'LineWidth',plt_vars.sig_width,'Color',roi_colors{roi_ix},...
                        'LineStyle','-'},plt_vars.errbar_alpha);
                end
                
                roi_lines{roi_ix} = tmp_line.mainLine;
                lgd_lab{roi_ix}   = [roi_list{roi_ix} '(n='...
                    num2str(size(ts{sr_ix}{roi_ix,cond_ix},1)) '/'...
                    num2str(sum(elec_roi_cnt(sr_ix,roi_ix))) ')'];
                sig_elec_cnt = sig_elec_cnt + size(ts{sr_ix}{roi_ix,cond_ix},1);
            elseif ~isempty(ts_avg{sr_ix}{roi_ix,cond_ix})
                tmp_line = shadedErrorBar(win_center{sr_ix},...
                    ts_avg{sr_ix}{roi_ix,cond_ix},var(ts{sr_ix}{roi_ix,cond_ix},[],1),...
                    {'LineWidth',plt_vars.sig_width,'Color',roi_colors{roi_ix},...
                    'LineStyle','-'},plt_vars.errbar_alpha);
        ...
        % Plotting parameters
        ax = gca;
        ax.Title.String  = [cond_lab{cond_ix} ' (sig/total n='...
            num2str(sig_elec_cnt) '/' num2str(sum(elec_roi_cnt(sr_ix,:))) '): ' event_lab{sr_ix}];
        if strcmp(cond_lab{cond_ix},'RT')
            ax.YLim          = ylims_rt;
            ax.YTick         = yticks_rt;
        ...
        ax.XLim          = [0,size(sig_idx{sr_ix}{1,numel(cond_lab)},2)];
        ax.XTick         = 0:plt_vars.x_step_sz*sample_rate:size(sig_idx{sr_ix}{1,numel(cond_lab)},2);
    ...
    % Save figure
    if save_fig
        fig_dir = ['/home/knight/hoycw/PRJ_Stroop/results/HFA/GRP_SR_ts/' stat_id '/' an_id_s '-' an_id_r '/'];
