function SBJ10c_HFA_GRP_summary_mtx_perc_actv_corrRT_ANOVA_ROI(SBJs,stat_id,pipeline_id,an_id,actv_win,plt_id,save_fig,fig_vis)
error('How can I do this across gROIs? Would need to be matrix per ROI, which is messy...');
...
% OUTPUTS:
%   Matrix with diagonal showing % electrodes showing: active, deactive, RT correlations, ANOVA factors
%   off-diagonals show overlap between individual factors (i.e., electrode showing both effects)
% clear all; %close all;
fig_filetype = 'png';
label_spacer = 0;
groi_label_spacer = '      ';
...
for sbj_ix = 1:numel(SBJs)
    ...
    if ~isempty(setdiff(einfo(:,1),stat{1}.label))
    ...
    same_start = round(hfa_actv.time(1)*uint8(sample_rate))==round(stat{1}.time(1)*uint8(sample_rate));
    same_end   = round(hfa_actv.time(end)*uint8(sample_rate))==round(stat{1}.time(end)*uint8(sample_rate));
    same_numel = size(hfa_actv.time,2)==size(stat{1}.time,2);
    if ~same_start || ~same_end || ~same_numel
        error('time axes are not the same across hfa analyses!');
    end
    if ~isempty(setdiff(stat{1}.label,hfa_actv.label));
    ...
    for ch_ix = 1:numel(stat.label)
        ...
        if ~isempty(strmatch(einfo(einfo_ix,3),groi_list,'exact'))  % Avoid bad gROI elecs
