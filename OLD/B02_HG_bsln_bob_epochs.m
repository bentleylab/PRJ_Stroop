...
y_common      = 0;                    % all plots share y_lim? 0/1
% plot_prop_con = 0;
...
for SBJ_ix = 1:length(SBJs)
    for data_id_ix = 1:length(data_ids{SBJ_ix})
        ...
        for ix = 1:length(cond_lab)
            ...
            %minmax{ix}
            ch_min = min([min(min(eval(['mean_stim.' cond_lab{ix}])))...
                min(min(eval(['mean_resp.' cond_lab{ix}])))]);
            ch_max = max([max(max(eval(['mean_stim.' cond_lab{ix}])))...
                max(max(eval(['mean_resp.' cond_lab{ix}])))]);
            if (abs(ch_min) > minmax) || (ch_max > minmax)
                minmax = max([abs(ch_min) ch_max]);
            end
        ...
        out_file = [data_id '_' analysis_id '_' cond_id grand_id '_Bob.ep' epoch_id '_'...
            trl_id '_bsln' bsln_id env_id smooth_id];
