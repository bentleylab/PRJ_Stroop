...
%         trial_info.response_time = manual.trial_info.response_time;
        ...
        for ix = 1:length(cond_lab)
            ...
            eval(['trial_mean.' cond_lab{ix} ' = squeeze(nanmean(trials(:,cond_idx.'...
                cond_lab{ix} '==1,:),2));']);
            % Standarad Error of Mean
            eval(['trial_sem.' cond_lab{ix} ' = squeeze(nanstd(trials_stim(:,cond_idx.'...
        ...
        %% Plot ERPs
        % Plotting parameters
        x_lab   = -plot_lim(1):x_step:plot_lim(2);
        ...
        for ch = 1:n_chan
            ...
            for cond_ix = 1:length(cond_lab)
                if n_chan==1
                    eval(['ebar{cond_ix} = shadedErrorBar([],'...
                        'mean_stim.' cond_lab{c_ix} '(win_on:win_off),'...
                        'sem_stim.' cond_lab{c_ix} '(win_on:win_off),'...
                        '{''Color'',[cond_colors{cond_ix}],''LineStyle'','...
                        'cond_style{cond_ix}},sem_alpha);']);
                else
                    eval(['ebar{cond_ix} = shadedErrorBar([],'...
                        'mean_stim.' cond_lab{c_ix} '(win_on:win_off),'...
                        'sem_stim.' cond_lab{c_ix} '(win_on:win_off),'...
            ...
            % Axes and Labels
            ax = gca;
            ax.XLim = [0,sum(plot_lim)];
            ax.XTick = 0:x_step:sum(plot_lim);
            ax.XTickLabel = x_lab;
            main_lines = [];
            for lab_ix = 1:length(cond_lab)
                cond_lab_legend{lab_ix} = eval(['[cond_lab{lab_ix} ''-'' num2str(sum(cond_idx.'...
                    cond_lab{lab_ix} '))];']);
                main_lines = [main_lines ebar{lab_ix}.mainLine];
            end
            main_lines = [main_lines event_line];
            if avg_channels == 1
                title(strcat(event,'-locked ',analysis_id,': mean',avg_ch_lab));
            else
                title(strcat(event,'-locked ',analysis_id,': ',header_ecog.channel_labels(ch)));
            end
            if ch==1
            ...
            for cond_ix = 1:length(cond_lab)
                if n_chan==1
                    imagesc(squeeze(trials(RT_sort_cond_idx,win_on:win_off)));
                else
                    imagesc(squeeze(trials(ch,RT_sort_cond_idx,win_on:win_off)));
                end
                scat = scatter(RTs_sort_cond(cond_start_pos(cond_ix)+1:cond_end_pos(cond_ix)),...
                        cond_start_pos(c_ix)+1:cond_end_pos(c_ix),{'Color',[cond_colors{c_ix}]});
            end
            !!!abandon this plot, it's dumb and doesn't make sense
            cbar = colorbar;
            caxis(clim);
            event_line = line([plot_lim_s(1) plot_lim_s(1)],ylim);%, 'k--');
            % Axes and Labels
            ax = gca;
