function SBJ10c_HFA_GRP_elec_cnt_nact_RT_ANOVA(SBJs,stat_id,an_id,actv_win,roi_id,atlas_id,gm_thresh,pipeline_id)%,save_out)
...
for sbj_ix = 1:numel(SBJs)
    ...
    tmp = load(actv_filename,'actv_ch'); actv_ch{sbj_ix} = tmp.actv_ch;
    ...
    tmp = load(elec_tis_fname); elec{sbj_ix} = tmp.elec; clear tmp;
    
    % Sort elecs by stat labels
    cfgs = []; cfgs.channel = stat.label;
    elec{sbj_ix}    = fn_select_elec(cfgs,elec{sbj_ix});
    roi_lab{sbj_ix} = fn_atlas2roi_labels(elec{sbj_ix}.atlas_label,atlas_id,roi_id);
    
    %% Collect SBJ results
    % Get GM probability from tissue labels {'GM','WM','CSF','OUT'}
    gm_perc{sbj_ix} = elec{sbj_ix}.tissue_prob(:,1);
    gm_bin{sbj_ix}  = elec{sbj_ix}.tissue_prob(:,1)>gm_thresh;
    ...
    clear SBJ SBJ_vars w2 stat qvals tmp
end

%% Print positive and negative findings across analyses
% prints: ch_lab roi_lab gm% gm_bin actv grp1 grp2 rt
int_spacer = strjoin(repmat({'%i'},[numel(grp_lab)+3 1]),'\t');
print_line1 = ['%s\t\t%s\t%.2f\t' int_spacer '\n'];
print_line2 = ['%s\t%s\t%.2f\t' int_spacer '\n'];
grp_nact_sig_cnt = 0;
grp_sig_cnt = 0;
grp_elec_cnt = 0;
grp_nact_sig_per = [];
for sbj_ix = 1:numel(SBJs)
    ...
    sig_cnt = 0;
    fprintf('============== %s ==============\n',SBJs{sbj_ix});
    fprintf('lab\t\troi\tgm\tgmbin\tactv\t%s\tRT\n',strjoin(grp_lab,'\t'));
    for ch_ix = 1:numel(elec{sbj_ix}.label)
        % Print line only for sig elecs
        if any(grp_sig{sbj_ix}(ch_ix,:))
            sig_cnt = sig_cnt + 1;
            if isempty(find(strcmp(elec{sbj_ix}.label{ch_ix},actv_ch{sbj_ix}))) %gm_bin{sbj_ix}(ch_ix)==0
                print_dest = 2; actv_code = 0;
                nact_sig_cnt = nact_sig_cnt + 1;
                nact_sig_elecs = [nact_sig_elecs elec{sbj_ix}.label(ch_ix)];
                nact_sig_per = [nact_sig_per gm_perc{sbj_ix}(ch_ix)];
            else
                print_dest = 1; actv_code = 1;
            end
            
            if numel(elec{sbj_ix}.label{ch_ix})>7
                fprintf(print_dest,print_line2,elec{sbj_ix}.label{ch_ix},roi_lab{sbj_ix}{ch_ix},...
                    gm_perc{sbj_ix}(ch_ix),gm_bin{sbj_ix}(ch_ix),actv_code,grp_sig{sbj_ix}(ch_ix,:));
            else
                fprintf(print_dest,print_line1,elec{sbj_ix}.label{ch_ix},roi_lab{sbj_ix}{ch_ix},...
                    gm_perc{sbj_ix}(ch_ix),gm_bin{sbj_ix}(ch_ix),actv_code,grp_sig{sbj_ix}(ch_ix,:));            end
%         else
%             fprintf(print_line,elec{sbj_ix}.label{ch_ix},roi_lab{sbj_ix}{ch_ix},gm_perc{sbj_ix}(ch_ix),...
%                 gm_bin{sbj_ix}(ch_ix),grp_sig{sbj_ix}(ch_ix,:));
        end
    end
    grp_nact_sig_cnt = grp_nact_sig_cnt + nact_sig_cnt;
    grp_nact_sig_per = [grp_nact_sig_per nact_sig_per];
    grp_elec_cnt = grp_elec_cnt + numel(elec{sbj_ix}.label);
...
%     fprintf(print_line,'Total',squeeze(gm_perc(sbj_ix,:)));
    fprintf('\nnact_sig: %i / %i elecs (%.2f )\n',nact_sig_cnt,numel(elec{sbj_ix}.label),...
        nact_sig_cnt/numel(elec{sbj_ix}.label));
    fprintf('\nnact_sig: %i / %i sig elecs (%.2f )\n',nact_sig_cnt,sig_cnt,...
        nact_sig_cnt/sig_cnt);
    fprintf('%s\n',strjoin(nact_sig_elecs,','));
    disp(nact_sig_per);
    fprintf('\n\n');
end
...
fprintf('============== GRP non-active counts ==============\n');
fprintf('\ngrp_nact_sig: %i / %i elecs (%.2f )\n',grp_nact_sig_cnt,grp_elec_cnt,...
        grp_nact_sig_cnt/grp_elec_cnt);
disp(mean(grp_nact_sig_per));
fprintf('\ngrp_nact_sig: %i / %i sig elecs (%.2f )\n',grp_nact_sig_cnt,grp_sig_cnt,...
        grp_nact_sig_cnt/grp_sig_cnt);
...
%     filename = [out_dir 'GRP_HFA_elec_cnt_nact_' stat_id '_' an_id '.mat'];
%     fprintf('Saving %s\n',filename);
