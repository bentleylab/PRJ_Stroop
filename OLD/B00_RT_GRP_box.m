...
%% Load data
RTs       = cell([1 numel(cond_lab)*(numel(SBJs)+1)]);
RT_means  = NaN([numel(SBJs) numel(cond_lab)]);
RT_groups = NaN([numel(cond_lab)*(numel(SBJs)+1) 2]);
for sbj_ix = 1:numel(SBJs)
    ...
    for cond_ix = 1:numel(cond_lab)
        RTs{cond_ix+numel(cond_lab)*(sbj_ix-1)} = trial_info.response_time(logical(fn_condition_index(cond_lab{cond_ix},...
                                                                        trial_info.condition_n)));
        RT_means(sbj_ix,cond_ix) = mean(RTs{cond_ix+numel(cond_lab)*(sbj_ix-1)});
        RT_groups(cond_ix+numel(cond_lab)*(sbj_ix-1),:) = [sbj_ix cond_ix];
    end
...
box_offsets  = linspace(-0.25,0.25,numel(cond_lab));
for sbj_ix = 1:numel(SBJs)
    ...
    b{an_ix} = bar(box_offsets+an_ix,diag(bar_vars(an_ix,:)),0.9,'stacked');
    for groi_ix = 1:numel(groi_list)
        ...
        s = scatter(scat_offsets+box_offsets(groi_ix)+an_ix,...
