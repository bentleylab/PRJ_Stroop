function SBJ10c_HFA_GRP_plot_SR_ROI_RTcorr_ANOVA(SBJs,stat_id,pipeline_id,an_id_s,an_id_r,...
...
for cond_ix = 1:numel(cond_lab)
    ...
    fig_name = ['GRP_ANOVA_' stat_id '_SR_' roi_id '_' cond_lab{cond_ix}];
    ...
    for sr_ix = 1:2
        ...
        for roi_ix = 1:numel(roi_list)
            for ch_ix = 1:size(sig_idx{sr_ix}{roi_ix,cond_ix},1)
                if strcmp(cond_lab{cond_ix},'RT')
                    ...
                    for sig_ix = 1:size(sig_chunks,1)
                        ...
                        line(sig_times,squeeze(ts{sr_ix}{roi_ix,cond_ix}(ch_ix,sig_times)),...
                    ...
                    for sig_ix = 1:size(sig_chunks,1)
                        ...
                        line(win_center{sr_ix}(sig_times),...
                            squeeze(ts{sr_ix}{roi_ix,cond_ix}(ch_ix,sig_times)),...
                            'Color',roi_colors{roi_ix},'LineStyle',plt_vars.sig_style,...
                            'LineWidth',plt_vars.sig_width);
                    end
                end
