...
data_ids   = {{'RAC_BP_data'}};%{'RC_WM'},{'LAC_WM'},{'LAC_WM'},{'RAC_WM'}};%{'IH_CA'},{'RAC_WM'}};%,};%
...
event        = 's';                    % 's'/'r': event to lock trials
...
% Plotting (Y axis)
plot_lim    = [500 500];
y_scale_id = strcat('_c',num2str(clim(1)),'.',num2str(clim(2)));
...
%         trial_info.response_time = manual.trial_info.response_time;
        ...
        for ch = 1:size(data_hg,1)
            ...
            trials(ch,:,:) = fn_epoch_bsln_combo(data_hg(ch,:),word_onset,resp_onset,buff_lim,trial_type,...
        ...
        RTs_sort_cond = {};
%         RT_sort_cond_idx = [];
%         cond_start_pos = NaN(length(cond_lab));
%         cond_end_pos = NaN(length(cond_lab));
...
%             cond_start_pos(ix) = length(RTs_sort_cond);
            eval(['RTs_sort_cond{ix} = RTs_sorted(logical(cond_idx.' cond_lab{ix} '));']);
%             eval(['RT_sort_cond_idx = vertcat(RT_sort_cond_idx,RT_sort_idx(logical(cond_idx.' cond_lab{ix} ')));']);
%             cond_end_pos(ix) = length(RTs_sort_cond);
...
%             end
        ...
        % Plot Condition ERPs
        out_file = [data_id '_' analysis_id '_trials_' cond_id grand_id '_Bob.ep' epoch_id '_'...
            trl_id '_bsln' bsln_id env_id smooth_id y_scale_id];
        n_plots = size(trials_stim,1);
        if n_plots > 2
            fig_height = 1;
        else
        ...
        for ch = 1:n_plots
            ...
            for c_ix = 1:length(cond_lab)
                if n_plots==1
                    imagesc(squeeze(trials_stim(rt_sort_cond_idx,stim_win_on:stim_win_off)));
                else
                    imagesc(squeeze(trials_stim(ch,RT_sort_cond_idx,stim_win_on:stim_win_off)));
                end
                scats{c_ix} = scatter(RTs_sort_cond(cond_start_pos(c_ix)+1:cond_end_pos(c_ix)),...
                        cond_start_pos(c_ix)+1:cond_end_pos(c_ix),{'Color',[cond_colors{c_ix}]});
            end
            !!!abandon this plot, it's dumb and doesn't make sense
            cbar = colorbar;
            caxis(clim);
            ...
            for lab_ix = 1:length(cond_lab)
                cond_lab_legend{lab_ix} = eval(['[cond_lab{lab_ix} '' RTs-'' num2str(sum(cond_idx.'...
                    cond_lab{lab_ix} '))];']);
            end
            main_handles = [scats{:} stim_line];
%             if avg_channels == 1
...
%             end
            if ch==1
                legend(main_handles,cond_lab_legend{:},'stim','Location','northeast');%,'inc-con'
            end
            ...
            for c_ix = 1:length(cond_lab)
                ...
                scatter(RTs_sort_cond(cond_start_pos(c_ix)+1:cond_end_pos(c_ix)),...
                        cond_start_pos(c_ix)+1:cond_end_pos(c_ix),{'Color',[cond_colors{c_ix}]});
            end
            ...
            main_handles = [];
            for lab_ix = 1:length(cond_lab)
                main_handles = [main_handles ebar{lab_ix}.mainLine];
            end
            main_handles = [main_handles rt_line_r];            
            if ch==1
                legend(main_handles,cond_lab_legend{:},'resp','Location','southeast');%,'inc-con'
            end
        end
