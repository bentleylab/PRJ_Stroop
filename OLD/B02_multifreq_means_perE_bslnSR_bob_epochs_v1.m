...
data_ids   = {{'LAC_WM'}};%{'RC_WM'},{'LAC_WM'},{'IH_CA'},{'LAC_WM'},{'RAC_WM'}};%,'RAC_BP_data'}};%{'RAC_WM'}};
...
% Analysis parameters
HG_type      = 'multiband';                     % only for HG: 'wideband', 'multiband'
...
plot_lim      = [1000 1000];            % ms to plot before the event and **after RT**
...
%         trial_info.response_time = manual.trial_info.response_time;
        ...
        for ch_ix = 1:n_chan
            ...
            out_file = [data_id '_' header_ecog.channel_labels{ch_ix} '_' all_analysis_id...
                '_means_' event '_' cond_name '_Bob.ep' epoch_id '_' trl_id...
            ...
            for an_ix = 1:length(analyses)
                ...
                subplot(length(analyses),1,an_ix);
                hold on;
                ebars = {};
                main_lines = [];
                for cond_ix = 1:length(cond_lab)
