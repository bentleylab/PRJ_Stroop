function SBJ10a_ANOVA_regressRT(SBJ,an_id,stat_id)
...
if rt_confound
    ...
    cfg_rt.model      = 'yes';
    cfg_rt.confound   = zscore(trial_info.response_time);
    hfa = ft_regressconfound(cfg_rt, hfa);
end
% OUTPUT:
%   .beta  = weights
%   .model = confounds * weights = X * X\Y
%   .stat  = t values
%   .prob  = p values, but they're fucked up! (max=2...)
