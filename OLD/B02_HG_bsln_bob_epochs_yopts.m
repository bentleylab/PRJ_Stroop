...
% Analysis Parameters
SBJs       = {'IR39'};%'IR31','IR32','IR35','IR39'};%};%
data_ids   = {{'RAC_WM'}};%{'LAC_WM'},{'IH_CA'},{'LAC_WM'},{'RAC_WM'}};%,};%
...
filt_type    = 'multiband';            % only for HG: 'wideband', 'multiband'
    ...
    for data_id_ix = 1:length(data_ids{SBJ_ix})
        ...
        for ix = 1:length(cond_lab)
            ...
            eval(['sem_stim.' cond_lab{ix} ' = squeeze(nanstd(trials_stim(:,cond_idx.'...
                cond_lab{ix} '==1,:),2)/sqrt(sum(cond_idx.' cond_lab{ix} ')));']);
