...
% Analysis Parameters
SBJs       = {'IR21','IR35','IR39'};%{'IR35'};%
data_ids   = {{'RC_WM'},{'LAC_WM'},{'RAC_WM'}};%{{'LAC_WM'}};%
...
% Analysis parameters
analysis     = 'ERP';                  % 'ERP', 'HG'
filt_type    = 'multiband';            % 'wideband', 'multiband'
...
for SBJ_ix = 1:length(SBJs)
    for data_id_ix = 1:length(data_ids{SBJ_ix})
        ...
        %% Format trials
        % Cut Trials
        trials_stim = NaN([size(data_hg,1),length(ok_epochs),buff_lim(1)+max_RT+buff_lim(2)+1]);
        trials_resp = NaN([size(data_hg,1),length(ok_epochs),buff_lim(1)+buff_lim(2)+1]);
        for ch = 1:size(data_hg,1)
            ...
            trials_stim(ch,:,:) = fn_bsln_epochs(squeeze(trials_stim(ch,:,:)),...
                repmat(buff_lim(1),length(ok_epochs)),bsln_lim,bsln_type);
        ...
        for ix = 1:length(cond_lab)
            ...
            eval(['cond_idx.' cond_lab{ix}...
                ' = fn_condition_index(cond_lab{ix}, trial_info.condition_n(ok_epochs));']);
        ...
        % Plot Condition ERPs
        out_file = [data_id '_' analysis_id '_' cond_id grand_id '_ep' epoch_id '_'...
        ...
        clear data_ecog header_ecog trial_info SBJ data_id ok_epochs word_onset...
