function SBJ10c_HFA_GRP_plot_SR_ROIavg_but_RTcorr_ANOVA(SBJs,stat_id,pipeline_id,an_id_s,an_id_r,...
                                                roi_id,plt_id,save_fig,fig_vis,fig_filetype)%,butterfly
!!! looks shitty
                                                % Plot time series for all significant effects by ROI
%   show all channels on top of each other
%   to be used as an inset!
% clear all; %close all;
% fig_filetype = 'svg';
...
for cond_ix = 1:numel(cond_lab)
    ...
    for sr_ix = 1:2
        ...
        % Plot effect size for channels
        for roi_ix = 1:numel(roi_list)
            if strcmp(cond_lab{cond_ix},'RT') && ~isempty(ts{sr_ix}{roi_ix,cond_ix})
                tmp_line = plot(1:numel(ts{sr_ix}{roi_ix,cond_ix}(1,:)),...
                    ts{sr_ix}{roi_ix,cond_ix},...
                    'Color',roi_colors{roi_ix},'LineStyle','-');
            elseif ~isempty(ts{sr_ix}{roi_ix,cond_ix})
