...
% Analysis Parameters
SBJs       = {'IR35','IR39'};%'IR21','IR31','IR35','IR39'};%'IR32','IR39'};%};%
ch_ids   = {{'all'},{'all'}};%{'RAM*','RIN*','RAC*','LOF*','ROF*'}};
...
sig_lim   = [0,500];               % start of the first and end of the last sig window to test over
...
sig_win_step = 10;                      % Step size to slide teh sig window forward
...
for SBJ_ix = 1:length(SBJs)
    ...
    sig_wins = [buff_lim(1)-sig_lim(1):sig_win_step:trial_len-buff_lim(2)+sig_lim(2)-sig_win_len];
    sig_wins = vertcat(sig_wins,sig_wins+sig_win_len);
%     sig_thresh = 0.05/length(sig_win_ends);
    
    sig_pvals = NaN([size(trials,1) size(sig_wins,2)]);
    sig_qvals = NaN([size(trials,1) size(sig_wins,2)]);
    sig_fdrs = NaN([size(trials,1) size(sig_wins,2)]);
    sig_times = {};
    for ch_ix = 1:size(trials,1)
        sig_times{ch_ix} = [];
        for sig_win_ix = 1:size(sig_wins,2)
            % Grab the mean of each trial within this window
            
            % Permute trial means within each window
            sig_win = sig_wins(1,sig_win_ix):sig_wins(2,sig_win_ix);
            [sig_pvals(ch_ix, sig_win_ix),~] = fn_sig_permute_labels_mean_diff(...
                mean(trials(ch_ix,cond_idx{1}==1,sig_win),3),...
                mean(trials(ch_ix,cond_idx{2}==1,sig_win),3),sig_nboots);
%             if sig_pvals(sig_win_ix)<sig_thresh
%                 sig_idx{ch_ix} = unique([sig_idx{ch_ix} sig_win]);
%             end
        end
        [sig_fdrs(ch_ix,:), sig_qvals(ch_ix,:)] = mafdr(sig_pvals(ch_ix,:));
        sig_ts = logical(sig_pvals(ch_ix,:)<0.05/size(sig_wins,2));
        % calculate number of consecutive windows to meet the cluster lent
        ...
        for chunk_ix = 0:length(chunk_edges)
            ...
            if (sig_ts(chunk_start)==1) && (sig_wins(2,chunk_end)-sig_wins(1,chunk_start)>sig_clust_len)
                sig_times{ch_ix} = [sig_times{ch_ix} sig_wins(1,chunk_start):sig_wins(2,chunk_end)];
            end
        end
    end
    sig_times_file = [SBJ_dir,'04_proc/',data_id '_' analysis_id '_' event '_' cond_id...
        '_Bob.ep' epoch_id '_' trl_id '_bsln' bsln_id env_id smooth_id '_sig_times.mat'];
    save(sig_times_file,'sig_times','sig_wins','sig_pvals','ch_ids','header_ecog','sig_fdrs','sig_qvals');
    ...
    out_file = [data_id '_' analysis_id '_trials_' event '_' cond_id grand_id...
        '_Bob.ep' epoch_id '_' trl_id '_bsln' bsln_id env_id smooth_id sig_id y_scale_id];
    if n_chan > 2
        fig_height = 1;
    else
        fig_height = n_chan/3;
    ...
    for ch = 1:n_chan
        ...
        % Significance marker
        scatter(sig_times{ch},zeros([1 length(sig_times{ch})]),'Marker','.','LineWidth',0.1,...
                    'MarkerEdgeColor','k');
        % Axes and Labels
        ax = gca;
        ax.XLim = [0,win_off];
