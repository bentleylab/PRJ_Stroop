...
% Analysis Parameters
SBJs       = {'IR21','IR31','IR35','IR39'};%'IR32','IR39'};%};%
data_ids   = {{'RC_WM'},{'LAC_WM'},{'LAC_WM'},{'RAC_WM'}};%{'IH_CA'},{'RAC_WM'}};%,};%
cond_lab   = {'CNI'};              % conditions to average
...
smooth_it    = 0;                      % smooth the result before averaging (0/1)
smooth_freq  = 10;
...
if length(cond_lab) == 1
    switch cond_lab{1}
        case 'pcon'
            ...
            cond_colors = {'k', 'k--', 'r','r--'};    % colors for cond_lab plotting
        case 'CNI'
            cond_id = 'CNI';
...
bsln_id  = ['_' bsln_type(1) num2str(bsln_lim(1)) '.' num2str(bsln_lim(2))];
...
for SBJ_ix = 1:length(SBJs)
    for data_id_ix = 1:length(data_ids{SBJ_ix})
        ...
        manual = load(strcat(SBJ_dir,'03_events/',data_id,'_trial_info.mat'));
        ...
        for ch = 1:size(data_hg,1)
            ...
            trials_stim(ch,:,:) = fn_bsln_epochs(squeeze(trials_stim(ch,:,:)),...
                repmat(buff_lim(1),length(good_epochs)),bsln_lim,bsln_type);
            trials_resp(ch,:,:) = fn_bsln_epochs(squeeze(trials_resp(ch,:,:)),...
                repmat(buff_lim(1),length(good_epochs)),bsln_lim,bsln_type);
        end
        
        %% ERPs by condition
        minmax = zeros([size(trials_stim,1) length(cond_lab) 2 2]);    %separate ylim per chan
        cond_idx = {};
