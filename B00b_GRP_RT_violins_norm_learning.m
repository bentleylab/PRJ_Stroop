function B00b_GRP_RT_violins_norm_learning(SBJs,plt_id,save_fig,fig_vis,fig_ftype)
...
% Process parameters
fig_dir  = strcat([root_dir 'PRJ_Stroop/results/RTs/GRP/learning/']);
if ~exist(fig_dir,'dir')
    mkdir(fig_dir);
end

n_splits = 2;
split_lab = {'First Half','Second Half'};
split_colors = {[0.2 0.2 0.2], [0.6 0.6 0.6]};

%% Load data
cni_idx  = cell([numel(SBJs) n_splits]);
pc_idx   = cell([numel(SBJs) n_splits]);
pCI_idx  = cell([numel(SBJs) n_splits]);
rts      = cell([numel(SBJs) n_splits]);
late_idx = cell([numel(SBJs) n_splits]);
sbj_idx  = cell([numel(SBJs) n_splits]);
...
for s_ix = 1:numel(SBJs)
    ...
    sbj_rts = zscore(trial_info.response_time);
    
    % Split rts in half
    split_ix  = round(quantile(1:numel(sbj_rts),1/n_splits));
    split_idx = ones(size(sbj_rts));
    for q_ix = 2:n_splits
        if q_ix==n_splits
            split_idx(split_ix(q_ix-1)+1:end) = q_ix;
        else
            split_idx(split_ix(q_ix-1)+1:split_ix(q_ix)) = q_ix;
        end
    end
    
    for q_ix = 1:n_splits
        rts{s_ix,q_ix} = sbj_rts(split_idx==q_ix);
        
        % Condition index within SBJ
        cni_idx{s_ix,q_ix} = zeros(size(trial_info.trial_n(split_idx==q_ix)));
        for cond_ix = 1:numel(cni_lab)
            cni_idx{s_ix,q_ix}(logical(fn_condition_index(cni_lab{cond_ix},...
                trial_info.condition_n(split_idx==q_ix)))) = cond_ix;
        end
        pc_idx{s_ix,q_ix} = zeros(size(trial_info.trial_n(split_idx==q_ix)));
        for cond_ix = 1:numel(pc_lab)
            pc_idx{s_ix,q_ix}(logical(fn_condition_index(pc_lab{cond_ix},...
                trial_info.condition_n(split_idx==q_ix)))) = cond_ix;
        end
        pCI_idx{s_ix,q_ix} = zeros(size(trial_info.trial_n(split_idx==q_ix)));
        for cond_ix = 1:numel(pCI_lab)
            pCI_idx{s_ix,q_ix}(logical(fn_condition_index(pCI_lab{cond_ix},trial_info.condition_n(split_idx==q_ix),'trial_info',trial_info))) = cond_ix;
        end
        
        % Create SBJ factor for ANOVA
        sbj_idx{s_ix,q_ix} = ones(size(rts{s_ix,q_ix}))*s_ix;
        
        % Check for RTs overlapping with stim onset or baseline
        late_idx{s_ix,q_ix} = false(size(rts{s_ix,q_ix}));
        q_trials = find(split_idx==q_ix);
        for t_ix = 2:length(trial_info.resp_onset(split_idx==q_ix))
            trl_ix = q_trials(t_ix);
            if trial_info.word_onset(trl_ix)-trial_info.resp_onset(trl_ix-1) ...
                    <= late_RT_cut*trial_info.sample_rate
                late_idx{s_ix,q_ix}(t_ix) = true;
            end
        end
        if any(late_idx{s_ix,q_ix})
            fprintf('%s: %i late trials detected\n',SBJ,sum(late_idx{s_ix,q_ix}));
        end
        rts{s_ix,q_ix}(late_idx{s_ix,q_ix})     = [];
        cni_idx{s_ix,q_ix}(late_idx{s_ix,q_ix}) = [];
        pc_idx{s_ix,q_ix}(late_idx{s_ix,q_ix})  = [];
        pCI_idx{s_ix,q_ix}(late_idx{s_ix,q_ix}) = [];
...
for q_ix = 1:n_splits
    ...
    [pval(q_ix,:), table, stats, terms] = anovan(vertcat(rts{:,q_ix}),...
        {vertcat(cni_idx{:,q_ix}), vertcat(pc_idx{:,q_ix}), vertcat(sbj_idx{:,q_ix})},...
...
%% CNI Violins
% Trial Type
fig_name = 'GRP_RT_violins_norm_CNI_learning';
figure('Name',fig_name,'units','normalized','outerposition',[0 0 0.8 0.5],'Visible',fig_vis);
for q_ix = 1:n_splits
    subplot(1,n_splits,q_ix); hold on; ax = gca;
    ...
    for cni_ix = 1:numel(cni_lab)
        for s_ix = 1:numel(SBJs)
            rt_grps{cni_ix} = [rt_grps{cni_ix}; rts{s_ix,q_ix}(cni_idx{s_ix,q_ix}==cni_ix)];
        end
    ...
    ax.Title.String    = ['Group Split ' num2str(q_ix) ' (n=' num2str(numel(SBJs)) ') RTs: CNI p=' num2str(pval(q_ix,1),'%.3f')];
...
fig_name = 'GRP_RT_violins_PC_learning';
figure('Name',fig_name,'units','normalized','outerposition',[0 0 0.8 0.5],'Visible',fig_vis);
for q_ix = 1:n_splits
    subplot(1,n_splits,q_ix); hold on; ax = gca;
    ...
    for pc_ix = 1:numel(pc_lab)
        for s_ix = 1:numel(SBJs)
            rt_grps{pc_ix} = [rt_grps{pc_ix}; rts{s_ix,q_ix}(pc_idx{s_ix,q_ix}==pc_ix)];
        end
    ...
    ax.YLabel.String   = 'RT (z-score)';
    ax.YLabel.FontSize = axis_sz;
    ax.Title.String    = ['Group Split ' num2str(q_ix) ' (n=' num2str(numel(SBJs)) ') RTs: PC p=' num2str(pval(q_ix,2),'%.3f')];
    ax.Title.FontSize  = title_sz;
...
%% Interaction Violins
fig_name = 'GRP_RT_violins_CNI-PC_interaction_learning';
if violin_horz
...
for q_ix = 1:n_splits
        ...
        for pc_ix = 1:numel(pc_lab)
            ...
            for s_ix = 1:numel(SBJs)
                rt_grps{grp_ix} = [rt_grps{grp_ix};...
                    rts{s_ix,q_ix}(pc_idx{s_ix,q_ix}==pc_ix & cni_idx{s_ix,q_ix}==cni_ix)];
            end
    ...
    ax.YLabel.FontSize = axis_sz;
    ax.Title.String    = ['Group Split ' num2str(q_ix) ' (n=' num2str(numel(SBJs)) ') RTs: CNI p=' num2str(pval(q_ix,1),'%.3f')...
        '; PC p=' num2str(pval(q_ix,2),'%.3f') '; CNI*PC p='  num2str(pval(q_ix,4),'%.3f')];
    ax.Title.FontSize  = title_sz;
...
%% CNI Violins: Split test within condition
% Trial Type
fig_name = 'GRP_RT_violins_norm_CNI_learning_block_compare';
figure('Name',fig_name,'units','normalized','outerposition',[0 0 0.8 0.5],'Visible',fig_vis);
for cni_ix = 1:numel(cni_lab)
    subplot(1,numel(cni_lab),cni_ix); hold on; ax = gca;
    
    % Separate data by CNI
    rt_grps      = cell([n_splits 1]);
    split_grps = cell([n_splits 1]);
    sbj_grps   = cell([n_splits 1]);
    split_legend = cell([n_splits 1]);
    for q_ix = 1:n_splits
        for s_ix = 1:numel(SBJs)
            rt_grps{q_ix} = [rt_grps{q_ix}; rts{s_ix,q_ix}(cni_idx{s_ix,q_ix}==cni_ix)];
            sbj_grps{q_ix} = [sbj_grps{q_ix}; sbj_idx{s_ix,q_ix}(cni_idx{s_ix,q_ix}==cni_ix)];
        end
        split_grps{q_ix} = ones(size(rt_grps{q_ix}))*q_ix;
        split_legend{q_ix} = [split_lab{q_ix} ' (n=' num2str(numel(rt_grps{q_ix})) ')'];
    end
    
    factor_lab = {'Split','SBJ'};
    % model_terms = [1 0 0; 0 1 0; 1 1 0];
    [pval, table, stats, terms] = anovan(vertcat(rt_grps{:}),...
        {vertcat(split_grps{:}), vertcat(sbj_grps{:})},...
        'model', 'interaction', 'random', 2,...%, 'sstype', 3% 'continuous', strmatch('RT',w2.cond),
        'varnames', factor_lab, 'display', 'off');
    % Output pvals = [Split, SBJ, Split*SBJ]

    % Plot Violins
    violins = violinplot(padcat(rt_grps{:}), split_lab,...
        'ViolinAlpha', violin_alpha, 'ShowData', false);
    ...
    for q_ix = 1:n_splits
        % Change scatter colors to mark condition
        violins(q_ix).ViolinColor = cni_colors{cni_ix};
        violins(q_ix).BoxPlot.FaceColor = cni_colors{cni_ix};
        violins(q_ix).EdgeColor = cni_colors{cni_ix};
        % Grab violin for legend
        legend_obj{q_ix} = violins(q_ix).ViolinPlot;
    end
    ax.FontSize        = tick_sz;
    ax.YLabel.String   = 'RT (z-score)';
    ax.YLabel.FontSize = axis_sz;
    ax.Title.String    = ['Group ' cni_lab{cni_ix} ' (n=' num2str(numel(SBJs)) ') RTs: p=' num2str(pval(1),'%.3f')];
    ax.Title.FontSize  = title_sz;
    legend([legend_obj{:}],split_legend,'FontSize',leg_sz,'Location','northwest');
    if violin_horz
        view([90 -90]);
    end
end
...
fig_name = 'GRP_RT_violins_PC_stroop_learning';
figure('Name',fig_name,'units','normalized','outerposition',[0 0 0.8 0.5],'Visible',fig_vis);
for q_ix = 1:n_splits
    subplot(1,n_splits,q_ix); hold on; ax = gca;
    ...
    for pc_ix = 1:numel(pc_lab)
        for s_ix = 1:numel(SBJs)
            con_rts = rts{s_ix,q_ix}(pc_idx{s_ix,q_ix}==pc_ix & cni_idx{s_ix,q_ix}==con_ix);
            inc_rts = rts{s_ix,q_ix}(pc_idx{s_ix,q_ix}==pc_ix & cni_idx{s_ix,q_ix}==inc_ix);
    ...
    ax.YLabel.FontSize = axis_sz;
    ax.Title.String    = ['Group Split ' num2str(q_ix) ' (n=' num2str(numel(SBJs)) ') Stroop RT Effect by PC: p=' num2str(pval,'%.3f')];
    ax.Title.FontSize  = title_sz;
...
%% Compute Stroop Effect Across Data Splits 
fig_name = 'GRP_RT_violins_split_stroop_learning';
figure('Name',fig_name,'units','normalized','outerposition',[0 0 0.8 0.5],'Visible',fig_vis);
hold on; ax = gca;

stroop = zeros([numel(SBJs) n_splits]);
design = zeros([numel(SBJs) n_splits]);
con_ix = find(strcmp(cni_lab,'C'));
inc_ix = find(strcmp(cni_lab,'I'));
for q_ix = 1:n_splits
    for s_ix = 1:numel(SBJs)
        con_rts = rts{s_ix,q_ix}(cni_idx{s_ix,q_ix}==con_ix);
        inc_rts = rts{s_ix,q_ix}(cni_idx{s_ix,q_ix}==inc_ix);
        stroop(s_ix,q_ix) = mean(inc_rts)-mean(con_rts);
    end
    design(:,q_ix) = q_ix;
end
...
% ANOVA Statistics
[pval, table] = anovan(stroop(:), design(:),...
    'model', 1, 'varnames', 'split', 'display', 'off');
...
% Adjust plot propeties
legend_obj = cell([n_splits 1]);
for q_ix = 1:n_splits
    % Change scatter colors to mark condition
    violins(q_ix).ViolinColor = split_colors{q_ix};
    violins(q_ix).BoxPlot.FaceColor = split_colors{q_ix};
    violins(q_ix).EdgeColor = split_colors{q_ix};
    % Grab violin for legend
    legend_obj{q_ix} = violins(q_ix).ViolinPlot;
end
ax.FontSize        = tick_sz;
ax.YLabel.String   = 'I-C RT (z-score)';
ax.YLabel.FontSize = axis_sz;
ax.Title.String    = ['Group (n=' num2str(numel(SBJs)) ') Stroop RT Effect by Split Half: p=' num2str(pval,'%.3f')];
ax.Title.FontSize  = title_sz;
legend([legend_obj{:}],split_lab,'FontSize',leg_sz,'Location','southwest');
if violin_horz
    view([90 -90]);
end
...
fig_name = 'GRP_RT_violins_PC_N_learning';
...
for q_ix = 1:n_splits
    ...
    for pc_ix = 1:numel(n_pc_lab)
        for s_ix = 1:numel(SBJs)
            tmp_pc_idx = pc_idx{s_ix,q_ix}==find(strcmp(pc_lab,n_pc_lab{pc_ix}));
            rt_grps{pc_ix}   = [rt_grps{pc_ix}; rts{s_ix,q_ix}(tmp_pc_idx & cni_idx{s_ix,q_ix}==n_ix)];
            sbj_n_idx{pc_ix} = [sbj_n_idx{pc_ix}; sbj_idx{s_ix,q_ix}(tmp_pc_idx & cni_idx{s_ix,q_ix}==n_ix)];
            pc_n_idx{pc_ix}  = [pc_n_idx{pc_ix}; pc_idx{s_ix,q_ix}(tmp_pc_idx & cni_idx{s_ix,q_ix}==n_ix)];
        end
    ...
    % Run ANOVA on Neutral Trials
    factor_lab = {'PC','SBJ'};
    error('why does this give F stat = 0 on q_ix = 1?');
    [pval, table] = anovan(vertcat(rt_grps{:}),...
        {vertcat(pc_n_idx{:}), vertcat(sbj_n_idx{:})},...
        'model', 'interaction', 'random', 2,...%, 'sstype', 3% 'continuous', strmatch('RT',w2.cond),
        'varnames', factor_lab, 'display', 'off');
    ...
    ax.YLabel.String   = 'RT (z-score)';
    ax.YLabel.FontSize = axis_sz;
    ax.Title.String    = ['Group Split ' num2str(q_ix) ' (n=' num2str(numel(SBJs)) ') RTs: N PC p=' num2str(pval(1),'%.3f')];
    ax.Title.FontSize  = title_sz;
    legend([legend_obj{:}],pc_legend,'FontSize',leg_sz,'Location','northwest');
    if violin_horz
        view([90 -90]);
