function A02_parse_events_CWH(data_id, data_file_str, log_dir_struct, photo_channel_n, mic_channel_n, rt_window, ignore_trials, plot_it, save_it)
% 
% data_id [str] -- uniquely identifies the subject
% data_file_str [str] -- full file name of the input ecog data file, also name of the output file
% log_dir_struct has:
%   .log_file_str [str] -- name of the log file with extension
%   .wav_file_dir [str] -- name of wav dir inside log dir
% photo_channel_n [int] -- channel number for the photodiode data
% mic_channel_n [int] -- channel number for the microphone data
% rt_window [two element array] -- earliest and latest response onset following the onset of the word presentation
% ignore_trials [array] -- trials to be ignored fromthe ecog data nd the log file data
% plot_it [0/1] -- optional. plot_it = 1 to plot detected events
% save_it [0/1] -- whetehr to save output
%
...
SBJ = data_id(1:strfind(data_id,'_')-1);
SBJ_dir = strcat('/home/knight/hoycw/PRJ_Stroop/data/',SBJ,'/');
log_dir_name = '00_raw';
preproc_dir_name = '02_preproc';
output_dir_name = '03_events';
output_file = strrep(data_file_str,'.mat','_trial_info.mat');

if(~exist(fullfile(helper_function_dir_name)))
  fprintf('\nERROR: Helper function directory does not exist.\n');
  return;
end
if(~exist(fullfile(SBJ_dir,preproc_dir_name,data_file_str)))
  fprintf('\nERROR: Cannot find data at [%s].\n', fullfile(SBJ_dir,preproc_dir_name,data_file_str));
  return;
end
if(~exist(fullfile(SBJ_dir,log_dir_name,log_dir_struct.log_file_str)))
...
data_path = fullfile(SBJ_dir,preproc_dir_name);
log_path = fullfile(SBJ_dir,log_dir_name);
%   create directory if needed
output_path = fullfile(SBJ_dir,output_dir_name);
...
min_event_length = 0.8 * s_rate;    %!!!CWH: knob to pull up top
...
[~, ~, data_shades] = read_photodiode(data_photo_d, min_event_length, 4);  %CWH: knob to pull up top
clear data_photo;
...
%% Save results
if save_it
    save(fullfile(output_path,output_file), 'data_id', 'trial_info');
end
