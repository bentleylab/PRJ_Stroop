function A04_spectral_decompose_CWH(SBJ, data_id, freq_band, freq_spacing_ratio, frac_bandwidth, phase_cutoff_freq)
...
for band_n = 1:n_bands
    ...
    if(freq_band_centers(band_n) < phase_cutoff_freq)
      save(fullfile(output_path,[data_id '_' num2str(band_n) '.mat']), 'header_ecog',...
          'ecog_data_amp', 'ecog_data_phs', 'trial_info', 'SBJ', '-v7');
    else
      save(fullfile(output_path,[data_id '_' num2str(band_n) '.mat']), 'header_ecog',...
          'ecog_data_amp', 'trial_info', 'SBJ', '-v7');
    end
    fprintf('\n');
