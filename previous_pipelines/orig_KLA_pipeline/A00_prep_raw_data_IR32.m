...
%   3) Plot data to get parameters for autmoatic cleaning
%   4) Pick WM references, plot to check for artifacts
%   5) Set up variable for preprocessing, save as .mat
clear all; close all;
...
% events start ~75, nurse interrupts after T4 in B7 ~770 until start again ~845, ends ~1160
analysis_time = {[62 1170]};

% Focusing on interhemispheric strip, but also two lateral frontal grids
possible_data_lab = {};
possible_refs_lab = {};
for ch_ix = 1:16
    possible_data_lab = {possible_data_lab{:} strcat('IHL',num2str(ch_ix))};
    possible_refs_lab = {possible_refs_lab{:} strcat('IHL',num2str(ch_ix))};
end
for ch_ix = 17:32
    possible_data_lab = {possible_data_lab{:} strcat('IHR',num2str(ch_ix))};
    % Remove IHR18 (ch_ix 30) because of a big spike ~750s
    if ch_ix ~= 18
        possible_refs_lab = {possible_refs_lab{:} strcat('IHR',num2str(ch_ix))};
    end
end
...
%% Define the rereferencing scheme (CAR)
