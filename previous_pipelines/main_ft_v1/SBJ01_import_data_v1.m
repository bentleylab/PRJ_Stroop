...
bad_ch_neg = fn_ch_lab_negate(preproc_vars.ch_lab.bad);
eeg_ch_neg = fn_ch_lab_negate(preproc_vars.ch_lab.eeg);
photod_ch_neg = fn_ch_lab_negate(preproc_vars.ch_lab.photod);
mic_ch_neg = fn_ch_lab_negate(preproc_vars.ch_lab.mic);

% Load Neural Data
cfg            = [];
cfg.dataset    = preproc_vars.raw_filename;
cfg.continuous = 'yes';
cfg.channel    = {'all',bad_ch_neg{:},eeg_ch_neg{:},photod_ch_neg{:},mic_ch_neg{:}};
data = ft_preprocessing(cfg);

% EEG data
if ~isempty(preproc_vars.ch_lab.eeg)
    cfg.channel = preproc_vars.ch_lab.eeg;
    eeg = ft_preprocessing(cfg);
end

% Load event data
cfg.channel = {preproc_vars.ch_lab.photod{:},preproc_vars.ch_lab.mic{:}};
evnt = ft_preprocessing(cfg);

%% Cut to analysis_time
if ~isempty(preproc_vars.analysis_time)
    for epoch_ix = 1:length(preproc_vars.analysis_time)
        epoch_len{epoch_ix} = diff(preproc_vars.analysis_time{epoch_ix});
        cfg_trim = [];
        cfg_trim.latency = preproc_vars.analysis_time{epoch_ix};
    ...
    data = data_pieces{1};
    data.time{1} = data.time{1}-preproc_vars.analysis_time{1}(1);
    evnt = evnt_pieces{1};
    evnt.time{1} = evnt.time{1}-preproc_vars.analysis_time{1}(1);
    if ~isempty(preproc_vars.ch_lab.eeg)
        eeg = eeg_pieces{1};
        eeg.time{1} = eeg.time{1}-preproc_vars.analysis_time{1}(1);
    end
    if length(preproc_vars.analysis_time)>1
        for epoch_ix = 2:length(preproc_vars.analysis_time)
            ...
            data.time{1} = horzcat(data.time{1},data_pieces{epoch_ix}.time{1}-...
                preproc_vars.analysis_time{epoch_ix}(1)+data.time{1}(end)+data.time{1}(2));
            ...
            evnt.time{1} = horzcat(evnt.time{1},evnt_pieces{epoch_ix}.time{1}-...
                preproc_vars.analysis_time{epoch_ix}(1)+evnt.time{1}(end)+evnt.time{1}(2));
            
            if ~isempty(preproc_vars.ch_lab.eeg)
                eeg.trial{1} = horzcat(eeg.trial{1},eeg_pieces{epoch_ix}.trial{1});
                eeg.time{1} = horzcat(eeg.time{1},eeg_pieces{epoch_ix}.time{1}-...
                    preproc_vars.analysis_time{epoch_ix}(1)+eeg.time{1}(end)+eeg.time{1}(2));
            end
        end
    end
end
...
cfg = [];
cfg.channel = preproc_vars.ch_lab.mic;
...
%% Save data
nrl_out_filename = strcat(import_dir,SBJ,'_',num2str(resamp_freq),'hz.mat');
save(nrl_out_filename, '-v7.3', 'data');

if ~isempty(preproc_vars.ch_lab.eeg)
    eeg_out_filename = strcat(import_dir,SBJ,'_eeg_',num2str(resamp_freq),'hz.mat');
    save(eeg_out_filename, '-v7.3', 'eeg');
end

evnt_out_filename = strcat(import_dir,SBJ,'_evnt.mat');
