%% Extract IR39 with fieldtrip pipeline
% NOTE: This script only loads the whole thing, resamples, and saves it out
% further preprocessing is done in ft_preproc_IR32.m
...
% Channel Selection
% probe_names = {'AG*','IHR*','IHL*','FPG*'};
bad_ch = {...
    'FTG27','FTG28','FTG29','FTG35','FTG36','FTG37','FTG34',...%epileptic
    'FTG7','FTG25','FTG40',...%bad/noisy
    'IHR20','IHR21','IHR30','IHR31',...% noisy/get entrained
    '---(1)','---(2)','---(3)','---(4)','---(5)','---(6)',...% Not real data
    'LSH','RSH','EDF Annotations',...% Not real data
    };
car_bad_ch = {'IHR27','IHR28','IHR18',...
    'FPG3','FPG4','FPG11','FPG12','FPG19','FPG20',...
    'AG4','AG10','AG19'}; %exclude from the CAR
eeg_ch = {};
% evnt_ch = {...
%     'DC01',...%photodiode
%     'DC02',...%microphone
...
evnt_ch_neg = fn_ch_lab_negate(evnt_ch);
cfg            = [];
cfg.dataset    = raw_filename;
cfg.continuous = 'yes';
cfg.channel    = {'all',bad_ch_neg{:},eeg_ch_neg{:},evnt_ch_neg{:}};
...
mic_ch = fn_ch_lab_add_prefix(mic_ch,'POL ');
mic_ch = fn_ch_lab_add_suffix(mic_ch,'-Ref');
cfg = [];
cfg.channel = mic_ch;
...
% cfg.dftfilter = notch_yn; % line noise removal using discrete fourier transform
