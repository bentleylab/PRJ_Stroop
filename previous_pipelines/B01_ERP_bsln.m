...
bsln_id  = [bsln_type(1) '_' num2str(bsln_lim(1)) '.' num2str(bsln_lim(2))];
...
if avg_channels == 1
    avg_ch_lab = strcat('{',header_ecog.channel_labels{1});
    if length(header_ecog.channel_labels) > 1
        for ch_lab_ix = 2:length(header_ecog.channel_labels)
            avg_ch_lab = [avg_ch_lab ',' header_ecog.channel_labels{ch_lab_ix}];
        end
    end
...
if smooth_it == 1
    for ch_ix = 1:size(data_ecog,1)
        data_ecog(ch_ix,:) = eegfilt(data_ecog(ch_ix,:), header_ecog.sample_rate, [], smooth_freq);
    end
end
   
%% Format trials
% Cut Trials
trials_stim = NaN([size(data_ecog,1),n_trials,buff_lim(1)+max_RT+buff_lim(2)+1]);
trials_resp = NaN([size(data_ecog,1),n_trials,buff_lim(1)+buff_lim(2)+1]);
for ch = 1:size(data_ecog,1)
    % Cut trials
    if strcmp(trial_type,'nanpad')
        trials_stim(ch,:,:) = fn_epoch_cuts_nanpad(data_ecog(ch,:),word_onset,resp_onset,buff_lim);
        trials_resp(ch,:,:) = fn_epoch_cuts_nanpad(data_ecog(ch,:),resp_onset,resp_onset,buff_lim);
    elseif strcmp(trial_type,'datapad')
        trials_stim(ch,:,:) = fn_epoch_cuts_datapad(data_ecog(ch,:),word_onset,resp_onset,buff_lim);
        trials_resp(ch,:,:) = fn_epoch_cuts_datapad(data_ecog(ch,:),resp_onset,resp_onset,buff_lim);
    else
...
% Plot Condition ERPs
out_file = [data_id '_ERP' grand_id '_ep' epoch_id '_' trl_id '_bsln' bsln_id smooth_id];
