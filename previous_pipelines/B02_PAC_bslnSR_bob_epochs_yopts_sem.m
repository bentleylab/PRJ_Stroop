...
% bsln_lim_s   = [250, -50];             % ms before and after event
% bsln_r_event = 's';                    % 's'/'r': event from which to baseline correct response-locked data
% bsln_lim_r   = [250, -50];             % ms before and after event
...
%         end
        ...
        %% Format trials
        % Cut Trials
        trials_stim = NaN([size(phs_diff,1),length(good_epochs),buff_lim(1)+max_RT+buff_lim(2)+1]);
        trials_resp = NaN([size(phs_diff,1),length(good_epochs),buff_lim(1)+buff_lim(2)+1]);
        for ch_ix = 1:n_chan
...
%         PLV_time = abs(mean(exp(1i*angle_diff),1));
        ...
        mean_stim = NaN([n_chan length(cond_lab) trial_len]);
        sem_resp  = NaN([n_chan length(cond_lab) trial_len]);
        ...
        for ix = 1:length(cond_lab)
            ...
            eval(['mean_stim.' cond_lab{ix} ' = squeeze(abs(nanmean(exp(1i*trials_stim(:,cond_idx.'...
                cond_lab{ix} '==1,:)),2)));']);
            eval(['mean_resp.' cond_lab{ix} ' = squeeze(abs(nanmean(exp(1i*trials_resp(:,cond_idx.'...
                cond_lab{ix} '==1,:)),2)));']);
...
%             end
        ...
        plv_diff = 
        [pval,~] = fn_PLV_time_surr_epoch_diff_rand_PLV_ts(
        %% Plot ERPs
        % Plotting parameters
...
%         y_bound      = ceil(minmax/y_bound_chunk)*y_bound_chunk;
        ...
        % Plot Condition ERPs
        out_file = [data_id '_PAC' phs_fband_id '.' amp_fband_id '_'...
            cond_name '_Bob.ep' epoch_id '_' trl_id...
            '_bsln' bsln_id smooth_id sem_id y_scale_id];
        ...
        for ch_ix = 1:n_plots
            % Plot Stim Locked
            subplot(n_plots,2,ch_ix*2-1); hold on;
%             ebar = {};
...
%                         '''FaceColor'',cond_colors{c_ix},''FaceAlpha'',sem_alpha,''LineStyle'',''none'');']);
                    eval(['plot(mean_stim.' cond_lab{c_ix} '(stim_win_on:stim_win_off),''Color'',cond_colors{c_ix});']);
                else
%                     eval(['ebar{c_ix} = shadedErrorBar([],'...
%                         'mean_stim.' cond_lab{c_ix} '(ch,stim_win_on:stim_win_off),'...
%                         'sem_stim.' cond_lab{c_ix} '(ch,stim_win_on:stim_win_off),'...
%                         '{''Color'',[cond_colors{c_ix}],''LineStyle'',cond_style{c_ix}},sem_alpha);']);
                    eval(['plot(mean_stim.' cond_lab{c_ix} '(ch,stim_win_on:stim_win_off),''Color'',cond_colors{c_ix});']);
                end
            end
...
%             else
            title(strcat('Stim Locked: ',header_ecog.channel_labels(ch_ix)));
%             end
            if ch_ix==1
            ...
            % Plot RT locked
            subplot(n_plots,2,ch_ix*2); hold on;
%             ebar = {};
            for c_ix = 1:length(cond_lab)
                if n_plots==1
%                     eval(['ebar{c_ix} = shadedErrorBar([],'...
...
%                         '{''Color'',[cond_colors{c_ix}],''LineStyle'',cond_style{c_ix}},sem_alpha);']);
                    eval(['plot(mean_resp.' cond_lab{c_ix} '(resp_win_on:resp_win_off),''Color'',cond_colors{c_ix});']);
                else
%                     eval(['ebar{c_ix} = shadedErrorBar([],'...
%                         'mean_resp.' cond_lab{c_ix} '(ch,resp_win_on:resp_win_off),'...
%                         'sem_resp.' cond_lab{c_ix} '(ch,resp_win_on:resp_win_off),'...
%                         '{''Color'',[cond_colors{c_ix}],''LineStyle'',cond_style{c_ix}},sem_alpha);']);
                    eval(['plot(mean_resp.' cond_lab{c_ix} '(ch,resp_win_on:resp_win_off),''Color'',cond_colors{c_ix});']);
                end
            end
...
%             else
            title(strcat('Resp Locked: ',header_ecog.channel_labels(ch_ix)));
%             end
%             main_lines = [];
%             for lab_ix = 1:length(cond_lab)
%                 main_lines = [main_lines ebar{lab_ix}.mainLine];
