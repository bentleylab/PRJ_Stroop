...
% Analysis Parameters
SBJs       = {'IR54'};%'IR21','IR31','IR32','IR35','IR39'};%'IR39'};%};%
ch_ids     = {{'LAC*','LOF*','RAC*','ROF*'}};%'LAC2-LAC3'}};%,{'RC_WM'},{'LAC_WM'},{'IH_CA'},{'LAC_WM'},{'RAC_WM'}};%,'RAC_BP_data'}};%{'RAC_WM'}};
...
buff_lim  = [2000, 2000];            % buffers are for cutting time series and then plotting
sig_lim   = [1000, 1000];
...
for SBJ_ix = 1:length(SBJs)
    ...
    SBJ = SBJs{SBJ_ix};
    data_id = strcat(SBJ,'_proc_bp.reref.bandstop');%strcat(SBJ,'_',data_ids{SBJ_ix}{data_id_ix});
    ...
    load(strcat(SBJ_dir,'03_events/',SBJ,'_trial_info.mat'));
    ...
    sig_pvals  = NaN([length(analyses) n_chan size(sig_wins,2)]);
    sig_times  = {};
    for an_ix = 1:length(analyses)
...
%             title(strcat(SBJ,'_',header_ecog.channel_labels{ch_ix},'_',analyses{an_ix}));
    ...
    for ch_ix = 1:n_chan
        ...
        out_file = [data_id '_' header_ecog.channel_labels{ch_ix} '_' all_analysis_id...
            ...
            '_bsln' bsln_id smooth_id '_sig' plot_lim_id y_scale_id];
