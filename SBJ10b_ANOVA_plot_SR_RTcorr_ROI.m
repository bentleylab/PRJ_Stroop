...
%% Plot Results
fig_dir = [root_dir 'PRJ_Stroop/results/HFA/' SBJ '/' stat_id '/SR/' an_id_s '-' an_id_r '/' roi_id '/'];
...
for cond_ix = 1:numel(cond_lab)
    ...
    for roi_ix = 1:numel(roi_list)
        fig_name = [SBJ '_ANOVA_' stat_id '_SR_' cond_lab{cond_ix} '_' roi_list{roi_ix}];
...
%                 lgd_lab = {grp_lab{:} 'corr(RT)'};
                ...
                if cond_ix <= numel(grp_lab)
                    ...
                    if any(squeeze(qvals{sr_ix}(cond_ix,ch_ix,:))<st.alpha) && strcmp(roi_lab{ch_ix},roi_list{roi_ix})
                        any_plot = 1;
...
%                 lgd_lab = {lgd_lab{:} 'RT'};
            ...
            % Plotting parameters
            ax.Title.String  = [roi_list{roi_ix} ': ' event_lab{sr_ix}];
            ax.Box           = 'off';
            ax.YLim          = ylims;
            ax.YTick         = yticks;
