function SBJ10c_HFA_GRP_summary_box_perc_GMlim_CSE_RT_ANOVA_ROI(SBJs,stat_id,pipeline_id,an_id,roi_id,...
...
% OUTPUTS:
%   box chart with % active, % deactivated, % RT correlations, % ANOVA factors
% clear all; %close all;
% fig_filetype = 'png';
error('didnt finish, boxplots in matlab suck balls');
label_spacer = 0;
groi_label_spacer = '      ';
...
fig_name = ['GRP_HFA_box_perc_GMlim' num2str(gm_thresh*100) '_CSE_' stat_id '_' roi_id '_' event];
...
box_vars = {cse_cnt, rt_cnt};
% box_vars  = zeros([2+numel(grp_lab) numel(roi_list)]);
for roi_ix = 1:numel(roi_list)
%     box_vars(1,roi_ix) = sum(cI_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
%     box_vars(1,roi_ix) = sum(cse_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
%     box_vars(2,roi_ix) = sum(rt_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
    for grp_ix = 1:numel(grp_lab)
%         box_vars(2+grp_ix,roi_ix) = sum(grp_cnt(:,roi_ix,grp_ix))/sum(elec_cnt(:,roi_ix));
        box_vars{2+grp_ix} = squeeze(grp_cnt(:,:,grp_ix));
    end
end
...
box_offsets  = linspace(-0.25,0.25,numel(roi_list));   %box for activation, deactivation, condition
for cond_ix = 1:2+numel(grp_lab)
    % Use "stacked" boxs that have empty elements to trick MATLAB into
    % thinking there are multiple elements, which lets me change properties of individual boxs
    b{cond_ix} = boxplot(box_vars{cond_ix},'PlotStyle','compact');
%     b{cond_ix} = boxplot(box_offsets+cond_ix,box_vars(cond_ix,:),'PlotStyle','compact');
    for roi_ix = 1:numel(roi_list)
...
%         s = scatter(scat_offsets(has_elecs)+box_offsets(roi_ix)+cond_ix,...
...
%% Save figure
if save_fig
    fig_dir = [root_dir 'PRJ_Stroop/results/HFA/GRP_summary_box_ROI/CSE_'...
