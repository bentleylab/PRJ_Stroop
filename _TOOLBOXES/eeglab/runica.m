...
rng('shuffle','twister'); % Reseed the random number generator
...
if biasflag & extended
    while step < maxsteps, %%% ICA step = pass through all the data %%%%%%%%%
        ...
        if wts_blowup | isnan(change)|isinf(change),  % if weights blow up,
            ...
            icaprintf(verb,fid,'step %d - lrate %5f, wchange %8.8f, angledelta %4.1f deg\t%s\n', ...
                                step,      lrate,     change, degconst*angledelta, datestr(now));
            %
            %%%%%%%%%%%%%%%%%%%% Save current values %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %
...
icaprintf(verb,fid,'Elapsed time: %4.0f seconds\n\n', etime(function_stop_time, function_start_time));
