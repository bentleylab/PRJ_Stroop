...
function [laplac, time] = eeg_laplac(EEG, paso, smoothing_constant, flexibility_constant)
...
matriz_g = zeros(n+1);
for i=1:n
    matriz_g(i,1:n) = g2(aux(i,1)*ones(n,1),aux(i,2)*ones(n,1),aux(:,1),aux(:,2),flexibility_constant);
    g(i,:) = gm_1(aux(i,1)*ones(n,1),aux(i,2)*ones(n,1),aux(:,1),aux(:,2),flexibility_constant);
end


% Add smoothing constant to diagonal (from CSD Toolbox)
lambda = smoothing_constant;
for e = 1:size(matriz_g,1);                   % add smoothing constant to diagonale
  matriz_g(e,e) = matriz_g(e,e) + lambda; 
end
...
function valor=g2(r1,r2,r3,r4,flexibility_constant)
...
for i=1:25
    aux2(i,:) = ((2*n(i)+1)./(n(i).*(n(i)+1)).^flexibility_constant)*ones(1,L);
end
    valor = 1/4/pi*sum(aux2.*P);
 %-----------------------------------------------------   
function valor=gm_1(r1,r2,r3,r4,flexibility_constant)
%g entrega la sumatoria hasta 7 con m=3 de la ec. (2) de la tecnical note del
...
for i=1:25
    aux2(i,:)=(2*n(i)+1)/(n(i)*(n(i)+1))^(flexibility_constant-1)*ones(1,L);
end
