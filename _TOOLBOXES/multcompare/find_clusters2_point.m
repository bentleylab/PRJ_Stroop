...
function [clust_membership, n_clust]=find_clusters2_point(tscores,thresh,chan_hood,thresh_sign)

n_chan = length(tscores);

global clust_ids; % channel x 1 matrix indicated which cluster each "voxel" belongs to

clust_ids=zeros(n_chan,1);

chan_matrix = 1:n_chan;
...
%Clear chan matrix to save memory (just in case)
clear chan_matrix
...
for a=1:n_above,
    ...
    if ~clust_ids(voxel_id)
        ...
        while sum(check_me>0)
            ...
            new = follow_clust(n_above,first,n_clust,above_thresh_ids,above_thresh_chans,chan_hood,n_chan);
            check_me(new)=1;
...
%% End of Main Function
    function [new_members] = follow_clust(n_above,current_voxel_id,current_clust_num,above_thresh_ids,above_thresh_chans,chan_hood,n_chan)
        ...
        %                      voxels into the original data matrix (channel x
        %                      1)
        ...
        for b=current_clust_num:n_above,
            if ~clust_ids(above_thresh_ids(b))
                ...
                if chan_dist == 1,
                    %if voxels are at neighboring channels
