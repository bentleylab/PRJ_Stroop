...
function [pval, t_orig, clust_info, seed_state, est_alpha]=clust_perm2_point(dataA,dataB,chan_hood,n_perm,fwer,tail,thresh_p,verblevel,seed_state,freq_domain)
...
if length(size(dataA))~=2 || length(size(dataB))~=2 
    error('dataA and dataB need to be two dimensional (chan x participant)')
end
[n_chan, n_subsA]=size(dataA);
[n_chan2, n_subsB]=size(dataB);
...
sm1=sum(x1,2);
mn1=sm1/n_subsA;
ss1=sum(x1.^2,2)-(sm1.^2)/n_subsA;

sm2=sum(x2,2);
mn2=sm2/n_subsB;
ss2=sum(x2.^2,2)-(sm2.^2)/n_subsB;
