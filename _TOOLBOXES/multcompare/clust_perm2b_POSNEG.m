function [pval, t_orig, clust_info, est_alpha]=clust_perm2b_POSNEG(real_diffs, null_diffs, muhats, sigmahats, chan_hood,fwer,tail,thresh_p)
%
...
if tail==0
    mx_clust_massPOSNEG=zeros(1,n_perm);
else
...
for perm=1:n_perm
    ...
    if tail==0,
        ...
        mx_clust_massPOSNEG(perm)=find_mx_mass(clust_ids,all_t,n_clust,1);
        
        %negative clusters
        [clust_ids, n_clust]=find_clusters(all_t,thresh_t,chan_hood,-1);
        mx_clust_massPOSNEG(perm)=mx_clust_massPOSNEG(perm)+find_mx_mass(clust_ids,all_t,n_clust,-1);
...
if tail==0,    
    ...
    mx_clust_mass=abs(mx_clust_massPOSNEG);
...
if tail==0,
    ...
    for a=1:n_clust,
        ...
        clust_p=mean(mx_clust_massPOSNEG>=clust_mass)*2; %multiply by two to correct for doing an upper AND lower tailed test
