...
function [smoothdata,filtwts] = X_eegfilt(data,srate,locutoff,hicutoff,epochframes,filtorder,revfilt,firtype)
...
elseif hicutoff > 0                %  lowpass filter
    if hicutoff/nyq < MINFREQ
        error('eegfilt() - lowpass cutoff freq must be > %g Hz',MINFREQ*nyq);
    end
...
parfor c=1:chans
  smoothdata(c,:) = filtfilt(filtwts,1,data(c,:));
end
