...
function [weights,sphere,meanvar,bias,signs,lrates,data,y] = X_runica_GPU(data,varargin) % NB: Now optionally returns activations as variable 'data' -sm 7/05
...
if(GPU_OK)
  % Using GPU
  GPU_CAST_FUN        = 'gpuArray(';   % CPU-GPU cast functions
  CPU_CAST_FUN        = 'gather';       % GPU-CPU cast functions
else
  % No GPU
  GPU_CAST_FUN        = '(';   % CPU-GPU cast functions
  CPU_CAST_FUN        = '';       % GPU-CPU cast functions
end
%                                 
...
elseif strcmp(sphering,'off') %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if ~weights % is starting weights not given
      ...
      weights = eye(ncomps,chans)*sphere;  %#ok<NASGU> % begin with the identity matrix
...
BI = eval(['block*' GPU_CAST_FUN 'eye(ncomps,ncomps,''double''))']);
weights = eval([GPU_CAST_FUN 'double(weights))']);
prevwtchange = eval([GPU_CAST_FUN 'zeros(chans,ncomps,''double''))']);
onesrow = eval([GPU_CAST_FUN 'ones(1,block,''double''))']);
bias = eval([GPU_CAST_FUN 'zeros(ncomps,1,''double''))']);
signs = eval(['diag(' GPU_CAST_FUN 'double(signs)))']);
oldsigns = eval([GPU_CAST_FUN 'zeros(size(signs),''double''))']);
old_kk = eval([GPU_CAST_FUN 'zeros(1,ncomps,''double''))']);
...
rand('state',sum(100*clock));  %#ok<RAND> % set the random number generator state to
...
if biasflag && extended
    while step < maxsteps, %%% ICA step = pass through all the data %%%%%%%%%
        ...
        for t=1:block:lastt, %%%%%%%%% ICA Training Block %%%%%%%%%%%%%%%%%%%
            ...
            if eval(['max(max(' CPU_CAST_FUN '(abs(weights))))']) > MAX_WEIGHT
                wts_blowup = 1;
                change = nochange;
            end
            if ~wts_blowup
                ...
                if extblocks > 0 && rem(blockno,extblocks) == 0,
                    ...
                    if eval([CPU_CAST_FUN '(signs) == ' CPU_CAST_FUN '(oldsigns)']),
        ...
        if ~wts_blowup
            ...
            change = eval([CPU_CAST_FUN '(delta*delta'')']); 
        end
        %
        ...
        if wts_blowup || isnan(change)|| isinf(change),  % if weights blow up,
            ...
            bias = eval([GPU_CAST_FUN 'zeros(ncomps,1,''double''))']);
            prevwtchange = eval([GPU_CAST_FUN 'zeros(chans,ncomps,''double''))']);
            delta = eval([GPU_CAST_FUN 'zeros(1,chans*ncomps,''double''))']);
            step = 0;                          % start again
            change = nochange;
            ...
            %
            if step> 2
                angledelta = eval(['acos(' CPU_CAST_FUN '(delta*olddelta'')/sqrt(change*oldchange))']);
            end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    weights = eval([CPU_CAST_FUN '(weights)']);
end
%% Compute ICA Weights
if biasflag && ~extended
  fprintf('\n\nNOT CODED FOR THIS YET\n\n'); return; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     while step < maxsteps, %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         timeperm=randperm(datalength); % shuffle data order at each step
% 
%         for t=1:block:lastt, %%%%%%%%% ICA Training Block %%%%%%%%%%%%%%%%%%%
...
bias  = eval([CPU_CAST_FUN '(bias(windex))']);		% reorder them
signs = diag(signs);          %#ok<NASGU> % vectorize the signs matrix
signs = eval([CPU_CAST_FUN '(signs(windex))']);    % reorder them
