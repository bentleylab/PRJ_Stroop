function SBJ10c_HFA_GRP_summary_bar_perc_GMlim_CSE_RT_ANOVA_ROI(SBJs,stat_id,pipeline_id,an_id,roi_id,...
...
fig_name = ['GRP_HFA_bar_perc_GMlim' num2str(gm_thresh*100) '_CSE_' stat_id '_' roi_id '_' event];
...
bar_vars  = zeros([2+numel(grp_lab) numel(roi_list)]);
for roi_ix = 1:numel(roi_list)
%     bar_vars(1,roi_ix) = sum(cI_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
    bar_vars(1,roi_ix) = sum(cse_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
    bar_vars(2,roi_ix) = sum(rt_cnt(:,roi_ix))/sum(elec_cnt(:,roi_ix));
    for grp_ix = 1:numel(grp_lab)
        bar_vars(2+grp_ix,roi_ix) = sum(grp_cnt(:,roi_ix,grp_ix))/sum(elec_cnt(:,roi_ix));
...
for cond_ix = 1:2+numel(grp_lab)
    ...
    b{cond_ix} = bar(bar_offsets+cond_ix,diag(bar_vars(cond_ix,:)),0.9,'stacked');
    for roi_ix = 1:numel(roi_list)
...
%% Save figure
if save_fig
    fig_dir = [root_dir 'PRJ_Stroop/results/HFA/GRP_summary_bar_ROI/CSE_'...
