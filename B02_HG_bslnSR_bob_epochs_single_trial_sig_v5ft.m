...
%             end
        ...
        sig_qvals(ch_ix,:) = mafdr(sig_pvals(ch_ix,:),'BHFDR',true);
        sig_ts = logical(sig_qvals(ch_ix,:)<0.05);
    ...
    for ch = 1:n_chan
        ...
        % Significance marker
        scatter(sig_times{ch}-plot_lim(1),zeros([1 length(sig_times{ch})]),'Marker','.','LineWidth',0.1,...
                    'MarkerEdgeColor','k');
        % Axes and Labels
        ax = gca;
