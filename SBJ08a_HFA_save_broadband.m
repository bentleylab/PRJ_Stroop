...
%     error('Stop using broadband and use multitapers you dummy!');
    ...
    cfgpp.lpfilter  = 'yes';
    cfgpp.lpfreq    = 150;
    roi_trl = ft_preprocessing(cfgpp,roi_trl);
    
    % Hilbert method to extract power envelope
    for ch_ix = 1:numel(roi.label)
        for trl_ix = 1:numel(roi_trl.time)
            roi_trl.trial{1}(ch_ix,:) = abs(hilbert(roi_trl.trial{1}(ch_ix,:)));
        end
    end
    hfa = roi_trl;
else
