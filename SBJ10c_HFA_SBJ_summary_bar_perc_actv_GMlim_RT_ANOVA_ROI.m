function SBJ10c_HFA_SBJ_summary_bar_perc_actv_GMlim_RT_ANOVA_ROI(SBJs,stat_id,pipeline_id,an_id,actv_win,roi_id,...
...
fig_name = ['GRPsbj_HFA_bar_perc_actv_GMlim' num2str(gm_thresh*100) '_' stat_id '_' roi_id '_' event];
figure('Name',fig_name,'units','normalized',...
    'outerposition',[0 0 0.5 0.6],'Visible',fig_vis);
[rc,~] = fn_num_subplots(numel(SBJs));
...
%     scat_vars = {actv_cnt, dact_cnt, rt_cnt};
    ...
    bar_edge  = cell([numel(roi_list) 1]);
    for roi_ix = 1:numel(roi_list)
        bar_vars(1,roi_ix) = actv_cnt(sbj_ix,roi_ix)/elec_cnt(sbj_ix,roi_ix);
        bar_vars(2,roi_ix) = dact_cnt(sbj_ix,roi_ix)/elec_cnt(sbj_ix,roi_ix);
        bar_vars(3,roi_ix) = rt_cnt(sbj_ix,roi_ix)/elec_cnt(sbj_ix,roi_ix);
        for grp_ix = 1:numel(grp_lab)
            bar_vars(3+grp_ix,roi_ix) = grp_cnt(sbj_ix,roi_ix,grp_ix)/elec_cnt(sbj_ix,roi_ix);
%             scat_vars{3+grp_ix} = squeeze(grp_cnt(:,:,grp_ix));
        end
        % If 3 or less electrodes, make edge color red
        if elec_cnt(sbj_ix,roi_ix)<=2
            bar_edge{roi_ix} = 'r';
        else
            bar_edge{roi_ix} = 'k';
        end
...
%     scat_offsets = linspace(-0.015,0.015,numel(SBJs));
    ...
    for an_ix = 1:3+numel(grp_lab)
        ...
        for roi_ix = 1:numel(roi_list)
            set(b{an_ix}(roi_ix),'FaceColor',roi_colors{roi_ix},'EdgeColor',bar_edge{roi_ix});
...
%                 scat_vars{an_ix}(has_elecs,roi_ix)./elec_cnt(has_elecs,roi_ix),50,'k*');
    ...
    if sbj_ix==2    % this is likely IR27, which has NO effects... ugh.
        if strcmp(event,'stim')
    ...
    ax.Title.String = SBJs{sbj_ix};%'Percentage of Electrodes Showing Significant Effects';
