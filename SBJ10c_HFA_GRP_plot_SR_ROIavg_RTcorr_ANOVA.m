function SBJ10c_HFA_GRP_plot_SR_ROIavg_RTcorr_ANOVA(SBJs,stat_id,pipeline_id,an_id_s,an_id_r,...
    ...
    for roi_ix = 1:numel(roi_list)
        for cond_ix = 1:numel(grp_lab)+1
            if cond_ix<=numel(grp_lab)
                ...
                for ch_ix = 1:size(ts{sr_ix}{roi_ix,cond_ix},1)
                    ...
                    for sig_ix = 1:size(sig_chunks,1)
                        ...
                        elseif sig_sign<0
                            ...
                            error('Really? ts_avg is really exactly zero???');
                        end
                    end
                    if ch_type(1) && ch_type(2)
                        rt_type_idx(ch_ix) = 3;
                    elseif ch_type(1)
                        rt_type_idx(ch_ix) = 1;
                    elseif ch_type(2)
                ...
                ts_avg{sr_ix}{roi_ix,cond_ix}(1,:) = mean(ts{sr_ix}{roi_ix,cond_ix}(rt_type_idx==1,:),1);
                ts_avg{sr_ix}{roi_ix,cond_ix}(2,:) = mean(ts{sr_ix}{roi_ix,cond_ix}(rt_type_idx==2,:),1);
                ts_avg{sr_ix}{roi_ix,cond_ix}(3,:) = mean(ts{sr_ix}{roi_ix,cond_ix}(rt_type_idx==3,:),1);
            end
        end
    end
end
    ...
    for sr_ix = 1:2
        ...
        for roi_ix = 1:numel(roi_list)
            if strcmp(cond_lab{cond_ix},'RT') && ~isempty(ts_avg{sr_ix}{roi_ix,cond_ix})
                tmp_line = plot(1:numel(ts_avg{sr_ix}{roi_ix,cond_ix}(1,:)),...
                    ts_avg{sr_ix}{roi_ix,cond_ix},'LineWidth',plt_vars.sig_width,...
