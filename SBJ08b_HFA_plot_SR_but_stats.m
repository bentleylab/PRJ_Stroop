function SBJ08b_HFA_plot_SR_but_stats(SBJ,conditions,an_id_s,an_id_r,plt_id,save_fig,fig_vis)
...
fig_dir = ['/home/knight/hoycw/PRJ_Stroop/results/HFA/' SBJ '/' conditions '/SR_but/' an_id_s '-' an_id_r '/'];
...
%     if plot_rc(1)>1; fig_height=1; else fig_height=0.33; end;
    ...
    for epoch_ix = 1:2
        for cond_ix = 1:numel(cond_lab)
            ...
            trials = squeeze(hfa{epoch_ix,cond_ix}.powspctrm(:,ch_ix,:,:));
    ...
    for epoch_ix = 1:2
        ...
        means = NaN([numel(cond_lab) size(hfa{epoch_ix,1}.powspctrm,4)]);
        var = NaN([numel(cond_lab) size(hfa{epoch_ix,1}.powspctrm,4)]);
        for cond_ix = 1:numel(cond_lab)
            means(cond_ix,:) = squeeze(mean(hfa{epoch_ix,cond_ix}.powspctrm(:,ch_ix,:,:),1));
            var(cond_ix,:) = squeeze(std(hfa{epoch_ix,cond_ix}.powspctrm(:,ch_ix,:,:),[],1)./...
                                                    sqrt(size(hfa{epoch_ix,cond_ix}.powspctrm,1)))';
        end
        ...
        if sum(stat{epoch_ix}.mask(ch_ix,:))>0
            ...
            if (size(stat{epoch_ix}.time,2)~=size(hfa{epoch_ix,1}.time,2)) || ...
                                (sum(stat{epoch_ix}.time==hfa{epoch_ix,1}.time)~=numel(stat{epoch_ix}.time))
                for chunk_ix = 1:size(sig_chunks,1)
                    sig_chunks(chunk_ix,1) = find(hfa{epoch_ix,1}.time==stat{epoch_ix}.time(sig_chunks(chunk_ix,1)));
                    sig_chunks(chunk_ix,2) = find(hfa{epoch_ix,1}.time==stat{epoch_ix}.time(sig_chunks(chunk_ix,2)));
                end
            end
