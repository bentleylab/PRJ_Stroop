function SBJ08a_HFA_actv_onset(SBJ,pipeline_id,an_id,actv_win)
% Calculates high frequency activity, computes onset relative to baseline
% via bootstrapping, then saves results
error('didnt actually write this script. whats the best way to do it?? difference from null shuffled distribution, second derivative, or the Vinitha/Brett way??');
% clear all; %close all;
% Set up paths
...
if strcmp(event_type,'resp')
    hfa = fn_realign_tfr_s2r(hfa,trial_info.response_time,stat_lim);
