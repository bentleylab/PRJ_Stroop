function SBJ11b_corr_HFA_cond_acE_plot_mat_ts(SBJ,pipeline_id,stat_id,an_id_main,plt_id,...
...
for e_ix = 1:numel(hfa_cond{1}.label)
    if any(strcmp(einfo(:,1),hfa_cond{1}.label{e_ix}))
        actv_ix(strcmp(einfo(:,1),hfa_cond{1}.label{e_ix})) = 1;
    end
end
...
ax_tick = 1:numel(hfa_cond{1}.label);
set(gca,'XLim',[0.5 numel(hfa_cond{1}.label)+0.5],'XTick',ax_tick,'XTickLabel',x_lab,'FontSize',plt_vars.font_sz);
set(gca,'YLim',[0.5 numel(hfa_cond{1}.label)+0.5],'YTick',ax_tick,'YTickLabel',y_lab,'FontSize',plt_vars.font_sz);
xlabel(einfo_fields{plt_vars.sort_vec(1)});
ylabel(einfo_fields{plt_vars.sort_vec(2)});

% Set color scale
max_corr = max(abs(diff_vals));
...
fprintf('=========================== Main ROI Averaged Correlation Matrix ===========================\n');
...
for roi_ix = 1:numel(rois)
    fprintf('\t%s: %.02f (%i / %i)\n',rois{roi_ix},roi_corr_cnt(roi_ix,roi_ix)/roi_elec_cnt(roi_ix,roi_ix),...
        roi_corr_cnt(roi_ix,roi_ix),roi_elec_cnt(roi_ix,roi_ix));
end
fprintf('\nACROSS ROIs:\n');
for roi_ix1 = 1:numel(rois)
    for roi_ix2 = roi_ix1+1:numel(rois)
        fprintf('\t%s-%s: %.02f (%i / %i)\n',rois{roi_ix1},rois{roi_ix2},roi_corr_cnt(roi_ix1,roi_ix2)/roi_elec_cnt(roi_ix1,roi_ix2),...
            roi_corr_cnt(roi_ix1,roi_ix2),roi_elec_cnt(roi_ix1,roi_ix2));
    end
end
fprintf('\n');
...
fprintf('=========================== Plotting Correlation Distribution ===========================\n');
fig_name = strcat(SBJ,'_corr_HFA_acE_hist_',plt_id);
figure('Name',fig_name,'Visible',fig_vis,'units','normalized','outerposition',plt_vars.fig_dim);
hold on;

%     ksdensity(diff_vals);
histogram(diff_vals,plt_vars.hist_bins,'Normalization','probability');
%     y_vals = ylim;
%     heights = rand(sum(qvals<plt_vars.sig_cut),1)*plt_vars.y_jitter*diff(y_vals)+mean(y_vals);
%     scatter(diff_vals(plot_pairs),heights);

%     subplot(2,2,sr_ix+2); hold on;
scatter(diff_vals(setdiff(1:numel(diff_vals),plot_pairs)),qvals(setdiff(1:numel(qvals),plot_pairs)),'k');
scatter(diff_vals(plot_pairs),qvals(plot_pairs),'r');
xlabel('Correlation');
...
% scatter(diff_vals{1}(ns_pairs),diff_vals{2}(ns_pairs),'k');
% scatter(diff_vals{1}(sig_pairs1),diff_vals{2}(sig_pairs1),'r','Marker','x');
% scatter(diff_vals{1}(sig_pairs2),diff_vals{2}(sig_pairs2),'r','Marker','+');
% scatter(diff_vals{1}(intersect(sig_pairs1,sig_pairs2)),diff_vals{2}(intersect(sig_pairs1,sig_pairs2)),'c','Marker','*');
...
if plot_ts
    ...
    for pair_ix = 1:numel(sig_pairs)
        ...
        fig_name = strcat(SBJ,'_corr_HFA_acE_SR_ts_',plt_id,'_',...
            hfa_cond{1}.label{epair_ix(1)},'-',hfa_cond{1}.label{epair_ix(2)});
        ...
        cond_info.name       = {hfa_cond{1}.label{epair_ix(1)} hfa_cond{1}.label{epair_ix(2)}};
        ...
        plot_info.title  = [hfa.label{epair_ix(1)} '+' hfa.label{epair_ix(2)}...
            ': r=' num2str(diff_vals(sig_pairs(pair_ix))) ',q=' num2str(qvals(sig_pairs(pair_ix)))];
