function B00a_SBJ_RT_violins_learning(SBJ,plt_id,fig_vis,save_fig,fig_ftype)
%% Plot SBJ RTs for give conditions, split into 1st/2nd half to look for learning:
...
% Split RTs by half
n_splits = 2;
split_ix = quantile(1:numel(rts),1/n_splits);

rt_splits = cell([n_splits 1]);
rt_splits{1} = rts(1:split_ix(1));
for q_ix = 2:n_splits
    if q_ix==n_splits
        rt_splits{q_ix} = rts(split_ix(q_ix-1)+1:end);
    else
        rt_splits{q_ix} = rts(split_ix(q_ix-1)+1:split_ix(q_ix));
    end
end
