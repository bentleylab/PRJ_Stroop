...
%% Build Design Matrix
design = cell([1 numel(groups)]);
levels = cell([1 numel(groups)]);
for grp_ix = 1:numel(groups)
    [levels{grp_ix}, ~, ~] = fn_condition_label_styles(groups{grp_ix});
...
% Compute statistics
w2.pval = sum(bsxfun(@ge,w2.boot,w2.trial),4)./n_boots; % sum(boots>real)/n_boots
w2.zscore   = norminv(1-w2.pval,0,1);
w2.bootmean = mean(w2.boot,4);
w2.bootstd  = std(w2.boot,[],4);
% w2 = rmfield(w2,'boot');
w2.zscore(isinf(w2.zscore)) = norminv(1-1/n_boots/2,0,1);
...
if rt_correlation
    save(out_fname,'-v7.3','w2','stat');
else
    save(out_fname,'-v7.3','w2');
end
