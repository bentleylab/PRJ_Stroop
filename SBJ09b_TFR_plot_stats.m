function SBJ09b_TFR_plot_stats(SBJ,conditions,an_id,plt_id,save_fig,fig_vis)
...
sample_rate = (numel(tfr{1}.time)-1)/(tfr{1}.time(end)-tfr{1}.time(1));
...
%% Plot results
fig_dir = ['/home/knight/hoycw/PRJ_Stroop/results/TFR/' SBJ '/' conditions '/' an_id '/'];
...
for ch_ix = 1:numel(stat.label)
    fig_name = [SBJ '_' conditions '_' an_id '_' tfr{cond_ix}.label{ch_ix}];
...
%         layout = ft_prepare_layout(cfg_lay,tfr_plot);
    ...
    if sum(squeeze(stat.mask(ch_ix,question,:)))>0
