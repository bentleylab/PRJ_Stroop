...
function [aLabel,labels,indArea]=anatomicLabelFS(pos, atlasFile,lookArround,atlasMatrix,atlasS,opt)
...
%                   [aLabel,labels,indArea]=anatomicLabelFS(pos, [],lookArround,atlasMatrix,atlasS)
%                   [aLabel,labels,indArea]=anatomicLabelFS(pos, [],lookArround,atlasMatrix,atlasS,radio)
% lookArraound  1/0 looks for the most probable Labels in a radio (cube
%               arround: pos +/- radio)
...
% opt.mode      look arround mode: 'mode' most probable (default)/ 'prob'  probabilities 
...
if lookArround==0 % atlas from exact pos
    for i=1:N;
...
elseif lookArround==1 %most probable atlas
    % to remove...
    % 0=unknown
...
%                     temp='';
%                     for j=1:length(u)
%                         temp= [temp labels{indLabels(j)} ' ' num2str(p(j)*100,2) '% -' ];
%                     end
%                     aLabel(i)={temp(1:end-2)};
