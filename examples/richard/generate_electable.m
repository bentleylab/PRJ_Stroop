function generate_electable(filename, varargin)
...
for e = 1:numel(elec.label) % electrode loop
  ...
  for a = 1:numel(atlas) % atlas loop
    ...
    if strcmp(name{a}, 'Desikan-Killiany') || ...
      ...
      cfg.roi           = elec.elecpos_fs(e,:); % from elec_nat
    else
    ...
    % enter anatomical labels
    [cnt, idx] = max(labels.count);
