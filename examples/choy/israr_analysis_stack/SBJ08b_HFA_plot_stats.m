...
%     subplot(plot_rc(1),plot_rc(2),ch_ix);
    ...
    for cond_ix = 1:numel(cond_lab)
        means(cond_ix,:) = squeeze(mean(hfa{cond_ix}.powspctrm(:,ch_ix,:,:),1));
    ...
    if sum(squeeze(stat.mask(ch_ix,1,:)))>0
        ...
        if (size(stat.time,2)~=size(hfa{1}.time,2)) || (sum(stat.time==hfa{1}.time)~=numel(stat.time))
            for chunk_ix = 1:size(sig_chunks,1)
                sig_chunks(chunk_ix,1) = find(hfa{1}.time==stat.time(sig_chunks(chunk_ix,1)));
                sig_chunks(chunk_ix,2) = find(hfa{1}.time==stat.time(sig_chunks(chunk_ix,2)));
            end
        end
