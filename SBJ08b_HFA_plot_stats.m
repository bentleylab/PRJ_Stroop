function SBJ08b_HFA_plot_stats(SBJ,conditions,an_id,plt_id,save_fig,fig_vis)
...
sample_rate = (numel(roi_hfa{1}.time)-1)/(roi_hfa{1}.time(end)-roi_hfa{1}.time(1));

% Trim data to plotting epoch
cfg_trim = [];
cfg_trim.latency = plt_lim;
roi_hfa{1} = ft_selectdata(cfg_trim,roi_hfa{1});
roi_hfa{2} = ft_selectdata(cfg_trim,roi_hfa{2});
...
%     subplot(plot_rc(1),plot_rc(2),ch_ix);
    ...
    means = NaN([numel(cond_lab) size(roi_hfa{1}.powspctrm,4)]);
    var = NaN([numel(cond_lab) size(roi_hfa{1}.powspctrm,4)]);
    for cond_ix = 1:numel(cond_lab)
        means(cond_ix,:) = squeeze(mean(roi_hfa{cond_ix}.powspctrm(:,ch_ix,:,:),1));
        var(cond_ix,:) = squeeze(std(roi_hfa{cond_ix}.powspctrm(:,ch_ix,:,:),[],1)./sqrt(size(roi_hfa{cond_ix}.powspctrm,1)))';
    end
