function SBJ08c_HFA_cond_ROI_onset_order(SBJs,conditions,pipeline_id,an_id,plt_id,save_fig,fig_vis)
...
mean_RTs = NaN([numel(cond_lab) numel(SBJs)]);
...
fig_name = ['GRP_HFA_cond_onsets_gROI_' event];
figure('Name',fig_name,'units','normalized',...
        'outerposition',[0 0 1 1],'Visible',fig_vis);
hold on;

% Plot Condition Difference Onsets per SBJ
median_line_height = 0.5;
rt_marker_width = 2;
rt_marker_size  = 80;
groi_y_offset = [-0.05 0 0.05];
for sbj_ix = 1:numel(SBJs)
    s = [];
    for groi_ix = 1:numel(groi_list)
        s(groi_ix) = scatter(cond_g_onsets{sbj_ix,groi_ix},...
            repmat(sbj_ix+groi_y_offset(groi_ix),[1 numel(cond_g_onsets{sbj_ix,groi_ix})]),50,'o','filled');
        set(s(groi_ix),'MarkerFaceColor',groi_colors{groi_ix},'MarkerEdgeColor','k');
    end
    r = [];
    for cond_ix = 1:numel(cond_lab)
        r(cond_ix) = scatter(mean_RTs(cond_ix,sbj_ix),sbj_ix,rt_marker_size,'+');
    ...
    for groi_ix = 1:numel(groi_list)
        line([median_g_onsets(sbj_ix,groi_ix) median_g_onsets(sbj_ix,groi_ix)],...
            [sbj_ix-median_line_height/2 sbj_ix+median_line_height/2],...
...
ax.YLim            = [0 numel(SBJs)+1];
ax.YTick           = 1:1:numel(SBJs);        % Ticks anywhere non-zero in plot_idx
ax.YTickLabel      = SBJs;
% ax.YTickLabelRotation = 45;
ax.YColor  = 'k';

ax.Title.String = 'Condition Difference Onsets by gROI';
...
% %% Plot ROI Results
% % Create and format the plot
% fig_name = ['GRP_HFA_cond_onsets_ROI_' event];
% figure('Name',fig_name,'units','normalized',...
%         'outerposition',[0 0 1 1],'Visible',fig_vis);
% hold on;
% clear s;
...
% median_line_height = 0.5;
% rt_marker_width = 2;
% roi_y_offset = linspace(-0.05,0.05,numel(roi_list));
% for sbj_ix = 1:numel(SBJs)
%     s = [];
%     for roi_ix = 1:numel(roi_list)
%         s(roi_ix) = scatter(cond_onsets{sbj_ix,roi_ix},...
%             repmat(sbj_ix+roi_y_offset(roi_ix),[1 numel(cond_onsets{sbj_ix,roi_ix})]),50,'o','filled');
%         set(s(roi_ix),'MarkerFaceColor',roi_colors{roi_ix},'MarkerEdgeColor','k');
%     end
%     r = scatter(mean_RTs(sbj_ix),sbj_ix,'k','square','LineWidth',rt_marker_width);
%     for roi_ix = 1:numel(roi_list)
...
% ax.YColor  = 'k';
% 
% ax.Title.String = 'Condition Difference Onsets by ROI';
% ax.Title.FontSize = 16;
% 
% %% Save figure
% if save_fig
