...
for ch_ix = 1:numel(stat{1}.label)
    ...
    for set_ix = 1:2
        ...
        [axs,grp_lines,rt_line] = plotyy(...
            repmat(win_center,size(grp_lab)),squeeze(w2{set_ix}.trial(:,ch_ix,:))',...
            1:numel(stat{set_ix}.time),squeeze(stat{set_ix}.rho(ch_ix,:,:))');
        ...
        if strcmp(event_lab{set_ix},'stim')
            ...
            event_line = line([find(stat{set_ix}.time==0) find(stat{set_ix}.time==0)],ylims1,...
            ...
            if grp_ix <= numel(grp_lab)
                if any(squeeze(qvals{set_ix}(grp_ix,ch_ix,:))<0.05)
                    ...
                    elseif strcmp(plt_vars.sig_type,'patch')
                        ...
                        fprintf('%s %s (%s) -- %i SIGNIFICANT CLUSTERS FOUND...\n',...
                            stat{set_ix}.label{ch_ix},grp_lab{grp_ix},event_lab{set_ix},size(sig_chunks,1));
                        for sig_ix = 1:size(sig_chunks,1)
                    ...
                    fprintf('%s %s (%s) -- NO SIGNIFICANT CLUSTERS FOUND...\n',...
                        stat{set_ix}.label{ch_ix},grp_lab{grp_ix},event_lab{set_ix});
                end
            else
                ...
                if sum(stat{set_ix}.mask(ch_ix,:))>0
                    ...
                    fprintf('%s RT (%s) -- SIGNIFICANT CLUSTERS FOUND, plotting with significance shading...\n',...
                        stat{set_ix}.label{ch_ix},event_lab{set_ix});%,size(sig_chunks,1));
                else
                    fprintf('%s RT (%s) -- NO SIGNIFICANT CLUSTERS FOUND, plotting without significance shading...\n',...
                        stat{set_ix}.label{ch_ix},event_lab{set_ix});
                end
            end
        end
        
        % Plotting parameters
        axs(1).Title.String  = [stat{set_ix}.label{ch_ix} ':' event_lab{set_ix}];
        axs(1).Box           = 'off';
        axs(1).YLim          = ylims1;
        axs(1).YTick         = yticks1;
        axs(1).YLabel.String = '% Variance Explained';
        axs(1).XLim          = [0,size(stat{set_ix}.time,2)];
        axs(1).XTick         = 0:plt_vars.x_step_sz*sample_rate:size(stat{set_ix}.time,2);
