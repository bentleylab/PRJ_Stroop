function B00_RT_SBJ_hist_CSE(SBJs,conditions,save_fig,fig_vis)
...
for sbj_ix = 1:numel(SBJs)
    % Trial Type
    fig_name = strcat(SBJs{sbj_ix},'_RT_hist_',conditions);
    figure('Name',fig_name,'units','normalized','outerposition',[0 0 1 1],'Visible',fig_vis);
    
    % Plot Normalized Histograms
    bins = [floor(min(vertcat(RTs{sbj_ix,:}))):bin_size:ceil(max(vertcat(RTs{sbj_ix,:})))];
...
%         subplot(numel(cond_lab),1,cond_ix);
        ...
        [~,pval] = ttest2([RTs{sbj_ix,1}],[RTs{sbj_ix,2}]);%,'Alpha',alpha
        ax.XLabel.String   = 'RT (sec)';
        ax.XLabel.FontSize = 14;
        ax.YLabel.String   = 'Proportion of RTs';
        ax.YLabel.FontSize = 14;
        ax.Title.String    = [cond_lab{cond_ix} ' (p=' num2str(pval) ')'];
